...
def _zone_proaktiv(*, karenz_h: int = 6) -> ZonenKonfig:
    """Zone des T-0286-Recency-Tests, als Helper fuer T-0443."""
    return ZonenKonfig(
        zone_id="zone-1", name="Waldtest", ventil_kanal=1,
        feuchte_schwelle_min=35.0, feuchte_schwelle_max=65.0,
        feuchte_kritisch=20.0,
...
@pytest.mark.asyncio
async def test_t0443_proaktiv_unterdrueckt_waehrend_laufendem_wasser():
    """T-0443, zweites Feature: dieselbe Blindstelle im T-0286-Recency-Guard
    des PROAKTIVEN Triggers. Anderes Fenster (`versickerungs_karenz_stunden`),
    gleiche Ursache -- ein offenes Ventil erzeugt kein SCHLIESSEN.

    Lage wie im Reachability-Test (proaktiv wuerde feuern), aber es laeuft
    gerade Wasser und das letzte SCHLIESSEN liegt VOR der 6-h-Karenz. Der
    Sensor-Nachlauf, den der Guard abdecken soll, hat noch nicht einmal
    begonnen. -> kein proaktiver Trigger.

    Bewusst eigener Test statt Anhaengsel an den Bypass-Tests: es ist ein
    anderes Feature mit eigener Karenz, und eine Aenderung am gemeinsamen
    Helper darf hier nicht unbemerkt durchrutschen.
    """
    zone = _zone_proaktiv()
    motor, speicher = _motor(zone, feuchte=45.0)
    motor._konfig = SimpleNamespace(
        ml_physik_diagnose=MlPhysikDiagnoseKonfig(aktiv=True),
        standorte=[],
    )
    speicher.live_lauf_states = [
        _live_state(vor_minuten=42, dauer_minuten=90),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.empfehlungs_typ == "kein_bedarf"
    assert empf.soll_bewaessern is False
...
@pytest.mark.asyncio
async def test_t0443_proaktiv_feuert_ohne_laufendes_wasser_weiter():
    """Kontrollbedingung: identische Lage ohne `live_lauf_state`. Der
    proaktive Trigger muss weiter feuern, sonst hat der Guard das Feature
    stillgelegt statt es zu praezisieren.
    """
    zone = _zone_proaktiv()
    motor, speicher = _motor(zone, feuchte=45.0)
    motor._konfig = SimpleNamespace(
...
@pytest.mark.asyncio
async def test_t0443_proaktiv_guard_auch_im_pruefe_kanal_pfad():
    """Der Recency-Guard steht an ZWEI Stellen: `vorhersage_zone` (Dashboard
    und Shadow-Push) und `_strategie_verdict_pro_zone` (der Pfad, den der
    scharfe Auto-Loop ueber `pruefe_kanal` nimmt). Bekanntes Fehlerpattern
    in diesem Projekt: parallele Blocker-Kaskaden, von denen nur eine
    gefixt wird. Deshalb hier explizit die zweite Stelle.
    """
    zone = _zone_proaktiv()
    motor, speicher = _motor(zone, feuchte=45.0)
    motor._konfig = SimpleNamespace(
    ...
    verdict = await motor._strategie_verdict_pro_zone(
        zone, 45.0, 0.0, 0.0, JETZT,
    )
    assert verdict.soll_bewaessern is False
    assert verdict.empfehlungs_typ == "kein_bedarf"

    # Kontrollbedingung: ohne laufendes Wasser feuert derselbe Pfad.
    speicher.live_lauf_states = []
    verdict_frei = await motor._strategie_verdict_pro_zone(
        zone, 45.0, 0.0, 0.0, JETZT,
    )
    assert verdict_frei.empfehlungs_typ == "praeventiv"


@pytest.mark.asyncio
async def test_vorhersage_zone_regen_erwartet():
    zone = _zone()
    # T-0439: 9 mm in 6 h. Frueher genuegten 3 mm (reiner mm-Schalter);
    # jetzt muss der Regen die Zone ueber die Schwelle heben koennen, damit
    # gesperrt wird. 4 pp/mm x (9-2) = 28 pp -> 25 wird zu 53.
    vorhersage = baue_vorhersage(regen_pro_stunde=1.5, anzahl_stunden=12)
...
async def test_vorhersage_zone_regen_erwartet_liefert_hypothetische_dauer():
    ...
    zone = _zone(ventil_kanal=1)
    # T-0439: s. Kommentar in test_vorhersage_zone_regen_erwartet.
    vorhersage = baue_vorhersage(regen_pro_stunde=1.5, anzahl_stunden=12)
    motor, _ = _motor(zone, feuchte=25.0, vorhersage=vorhersage)
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.blocker_typ == BlockerTyp.REGEN_ERWARTET
...
# ---------- T-0443: offenes Ventil blockt den Kritisch-Bypass ----------

def _offener_lauf(
    *, vor_minuten: int, dauer_minuten: int, kanal: int = 1,
    geraet_id: str = "dswc-1",
) -> list[VentilEreignis]:
    """Ein LAUFENDER Lauf: OEFFNEN ohne SCHLIESSEN. Genau die Konstellation,
    fuer die der Event-Guard blind ist -- er zaehlt nur SCHLIESSEN."""
    return [
        VentilEreignis(
            zeitstempel=JETZT - timedelta(minutes=vor_minuten),
            zone_id="zone-1", ventil_id=f"{geraet_id}:{kanal}",
            aktion=VentilAktion.OEFFNEN, dauer_sekunden=0,
            ausloser=Ausloser.MANUELL,
        ),
    ]
...
def _live_state(
    *, vor_minuten: int, dauer_minuten: int, kanal: int = 1,
    geraet_id: str = "dswc-1", zone_ids: list[str] | None = None,
) -> dict:
    return {
        "kanal": kanal,
        "valve_id": f"{geraet_id}:{kanal}",
        "geraet_id": geraet_id,
        "zone_ids": zone_ids if zone_ids is not None else ["zone-1"],
...
@pytest.mark.asyncio
async def test_t0443_offenes_ventil_blockt_bypass():
    """DER FIX. Realfall 28.07. 05:14: manueller Pre-Soak, Puls-Close 04:12,
    Hauptlauf seit 04:32 OFFEN. Das letzte SCHLIESSEN lag 2,5 min VOR dem
    1-h-Fenster [04:14, 05:14] -> Event-Guard sagt "nicht kuerzlich
    gegossen" -> Bypass -> Push "giesse 90 min", waehrend Wasser lief.

    Nachgebaut: Puls-Lauf beendet vor 62 min (ausserhalb der 1-h-Karenz),
    Hauptlauf seit 42 min offen. Ohne den Kanal-Check faellt das durch.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ereignisse = (
        _lauf(vor_minuten=67, dauer_minuten=5)      # Puls, Close vor 62 min
        + _offener_lauf(vor_minuten=42, dauer_minuten=90)
    )
    motor, speicher = _motor(
        zone, feuchte=15.0, heutige_ereignisse=ereignisse,
    )
    speicher.live_lauf_states = [
        _live_state(vor_minuten=42, dauer_minuten=90),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.blocker_typ == BlockerTyp.PAUSE_AKTIV, (
        "waehrend Wasser laeuft darf der Bypass nicht feuern"
    )
    assert empf.pause_bypass_kritisch_aktiv is False


@pytest.mark.asyncio
async def test_t0443_ohne_kanal_check_waere_der_bypass_gefeuert():
    """Kontrollbedingung zum Test darueber: identische Lage, nur ohne
    `live_lauf_state`. Dann greift wieder das alte Verhalten (Bypass).
    Faellt dieser Test um, testet der Test darueber etwas anderes als den
    Kanal-Check -- z.B. den Event-Guard, der hier bewusst nicht greift.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ereignisse = (
        _lauf(vor_minuten=67, dauer_minuten=5)
        + _offener_lauf(vor_minuten=42, dauer_minuten=90)
    )
    motor, speicher = _motor(
        zone, feuchte=15.0, heutige_ereignisse=ereignisse,
...
@pytest.mark.asyncio
async def test_t0443_schwesterzone_am_selben_kanal_blockt_auch():
    """Kanal statt Zone ist tragend: bambuswald und bambuswald_yogaraum
    haengen seriell am selben Kanal (DSWC1/K2). Laeuft das Ventil unter der
    SCHWESTERZONE, ist eine zone_id-Abfrage blind -- Wasser fliesst
    trotzdem durch beide.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ...
    # Kein Event auf zone-1; der Lauf laeuft unter der Schwesterzone.
    speicher.live_lauf_states = [
        _live_state(vor_minuten=42, dauer_minuten=90, zone_ids=["zone-2"]),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is False
...
@pytest.mark.asyncio
async def test_t0443_anderer_kanal_blockt_nicht():
    """Gegenprobe: ein Lauf auf einem FREMDEN Kanal darf den Bypass nicht
    blocken, sonst legt jede beliebige Bewaesserung im Garten die
    Notpfade aller Zonen still."""
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    motor, speicher = _motor(
    ...
    speicher.live_lauf_states = [
        _live_state(vor_minuten=42, dauer_minuten=90, kanal=2),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is True
...
@pytest.mark.asyncio
async def test_t0443_veralteter_live_state_blockt_nicht_ewig():
    """Frische-Schranke (Entscheidung Andre 28.07.): eine haengengebliebene
    `live_lauf_state`-Zeile (Realfall 02.07.: 12 h Geister-"laeuft") wuerde
    Bypass UND Shadow-Push dauerhaft stummschalten -- genau die Notpfade,
    die bei Trockenheit greifen sollen.

    Lauf mit 30 min Dauer, gestartet vor 5 h: planmaessiges Ende plus
    15 min Toleranz ist lange durch -> Zeile wird ignoriert.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ...
    speicher.live_lauf_states = [
        _live_state(vor_minuten=300, dauer_minuten=30),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is True
...
def _pre_soak_state(
    *, vor_minuten: int, pause_min: int = 30, haupt_min: int = 60,
    pre_soak_min: int = 5, phase: str = "pause",
    zone_ids: list[str] | None = None, haupt_pulse: int = 1,
    haupt_pause_min: int = 0,
) -> dict:
    return {
        "zone_id": "zone-1",
        "kanal": 1,
        ...
        "haupt_pause_s": haupt_pause_min * 60,
        "haupt_pulse_gestartet": 0,
    }


@pytest.mark.asyncio
async def test_t0446_soak_pause_blockt_bypass():
    """T-0446: waehrend der Soak-Pause ist das Ventil ZU, `live_lauf_state`
    also leer -- der Vorgang laeuft aber weiter (Bambus: 25 min zwischen
    Puls und Hauptlauf). Der scharfe Auto-Loop ueberspringt den Kanal ueber
    die ganze Sequenz (`main.py` prueft den Pre-Soak-Lauf); ohne diesen
    Check haetten Shadow-Push und scharfes Verhalten verschiedene
    Wahrheiten -- genau der Drift, den der Shadow-Docstring ausschliesst.

    Sequenz vor 20 min gestartet: Puls 5 min, Haupt-Start bei 30 min.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ...
    speicher.live_lauf_states = []          # Ventil ist zu
    speicher.pre_soak_states = [_pre_soak_state(vor_minuten=20)]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is False


@pytest.mark.asyncio
async def test_t0446_beendete_sequenz_blockt_nicht():
    """Kontrollbedingung: nach dem Sequenz-Ende plus Toleranz ist die Zeile
    ein Geist und darf den Bypass nicht laenger unterdruecken.

    Ende der Sequenz bei 30 + 60 = 90 min, Toleranz 15 min.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ...
    speicher.pre_soak_states = [_pre_soak_state(vor_minuten=200)]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is True


@pytest.mark.asyncio
async def test_t0446_endphase_zaehlt_nicht_als_laufend():
    """Eine Zeile in einer Endphase ist kein laufender Vorgang, auch wenn
    sie zeitlich noch im Fenster liegt (Loesch-Versuch kann scheitern)."""
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    motor, speicher = _motor(
    ...
    speicher.pre_soak_states = [
        _pre_soak_state(vor_minuten=20, phase="fehler"),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is True
...
@pytest.mark.asyncio
async def test_t0446_schwesterzone_ueber_zone_ids_kanal():
    """`pre_soak_state` hat keine `geraet_id`, ein Match auf die blosse
    Kanalnummer wuerde bei zwei DSWCs mit gleicher Nummer falsch
    anschlagen. Zugehoerigkeit laeuft deshalb ueber `zone_ids_kanal` --
    hier laeuft die Sequenz unter der Schwesterzone.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ...
    fremd = _pre_soak_state(vor_minuten=20, zone_ids=["zone-1", "zone-2"])
    fremd["zone_id"] = "zone-2"
    speicher.pre_soak_states = [fremd]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is False

    # Gegenprobe: eine Sequenz, zu der diese Zone nicht gehoert, blockt nicht.
    fremd["zone_ids_kanal"] = ["zone-2"]
    empf2 = await motor.vorhersage_zone("zone-1")
    assert empf2.pause_bypass_kritisch_aktiv is True


@pytest.mark.asyncio
async def test_t0443_lauf_innerhalb_toleranz_blockt_noch():
    """Die Gegenprobe zur Frische-Schranke: kurz NACH dem planmaessigen
    Ende (Cloud-Latenz, Watchdog-Tick) gilt die Zeile weiter. Sonst waere
    die Schranke ein Loch statt einer Begrenzung.

    30-min-Lauf vor 40 min gestartet: Ende vor 10 min, Toleranz 15 min.
    """
    zone = _zone_pause_bypass(
        min_pause_minuten=120, karenz_h=3, bypass_karenz_h=1.0,
    )
    ...
    speicher.live_lauf_states = [
        _live_state(vor_minuten=40, dauer_minuten=30),
    ]
    empf = await motor.vorhersage_zone("zone-1")
    assert empf.pause_bypass_kritisch_aktiv is False


# --- T-0560: Panel spiegelt die Blocker des Entscheidungspfads ---

@pytest.mark.asyncio
async def test_t0560_konvektions_guard_greift_auch_im_panel():
    """Hohe Regenwahrscheinlichkeit bei geringer mm-Prognose -> REGEN_ERWARTET.

    `pruefe_zone` hat diesen Guard seit T-0322 (open-meteo unterschaetzt
    Konvektion notorisch). `vorhersage_zone` hatte ihn nicht, obwohl sie die
    Blocker des Entscheidungspfads spiegeln soll. Folge: die Engine sperrte,
    das Panel lieferte `soll_bewaessern=True` samt Dauer -- und der Watchdog
    baute daraus einen Push.
    """
    zone = _zone(ventil_kanal=1, optimum_min=40.0)
    # 0.1 mm/h -> 0.6 mm in 6 h, also unter der mm-Schwelle des Regen-Gates;
    # allein die Wahrscheinlichkeit darf sperren.
    motor, _ = _motor(
        zone, feuchte=25.0,
    ...
    assert empf.blocker_typ == BlockerTyp.REGEN_ERWARTET
    assert empf.soll_bewaessern is False
    assert "Starkregen" in (empf.grund or "")


@pytest.mark.asyncio
async def test_t0560_niedrige_regenwahrscheinlichkeit_blockt_nicht():
    """Gegenprobe: unterhalb der Schwelle bleibt der Positiv-Pfad erhalten.

    Ohne diesen Fall koennte der Guard beliebig scharf gestellt werden, ohne
    dass ein Test es merkt.
    """
    # optimum_min wie im Positiv-Pfad-Bestandstest -- sonst ist der Fall
    # `kein_bedarf` und die Gegenprobe wuerde aus dem falschen Grund gruen.
    zone = _zone(ventil_kanal=1, optimum_min=40.0)
    motor, _ = _motor(
        zone, feuchte=25.0,
        vorhersage=baue_vorhersage(
            regen_pro_stunde=0.1, regen_wahrscheinlichkeit=20.0,
        ),
    )
...
@pytest.mark.asyncio
async def test_t0560_folge_dose_ohne_kanalrate_stuerzt_nicht():
    """Mehrfach-Takt ohne Liter-Rate -> kein TypeError.

    `_liter_fuer_dauer` liefert None ohne BilanzKonfig, ohne `ventil_kanal`
    oder ohne Rate fuer den Kanal; `round(None)` riss Dashboard-Snapshot,
    Audit-Snapshot und Shadow-Push dieser Zone mit. Erreicht wird der Pfad,
    sobald die rohe Dauer `max_dauer_sekunden` uebersteigt.
    """
    # Kanal 7 existiert in der Default-Bilanz nicht -> Rate None.
    zone = _zone(ventil_kanal=7, max_dauer_sekunden=60)
    motor, _ = _motor(zone, feuchte=5.0)

    empf = await motor.vorhersage_zone("zone-1")

    assert empf.folge_dose_liter is None
    assert empf.folge_dose_dauer_s is None or empf.folge_dose_dauer_s > 0
