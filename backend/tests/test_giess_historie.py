...
def test_t0566_manueller_handguss_ist_kein_dauerlauf():
    """`POST /api/giessen` schreibt EIN OEFFNEN mit Dauer, kein SCHLIESSEN.

    Das ist eine dokumentierte Ausnahme des Ventil-Event-Vertrags (der
    Nutzer loggt eine Giesskannen-Aktion nach, es gibt kein Ventil).
    `_offener_puls_dict` setzt fuer ein OEFFNEN ohne Pendant aber hart
    `dauer_s: None` -- die eingetragene Dauer verschwand aus der Historie
    und `laeuft_noch` blieb dauerhaft True. Aufraeumen konnte das niemand:
    `hole_orphan_oeffnen` und `hole_offene_ventil_kanaele` filtern auf
    `ventil_id GLOB '*:[0-9]*'`, und `manuell` matcht da nicht.
    """
    e = VentilEreignis(
        zeitstempel=datetime(2026, 9, 1, 9, 0),
        zone_id="zitrus", ventil_id="manuell",
        aktion=VentilAktion.OEFFNEN, dauer_sekunden=600,
        ausloser=Ausloser.MANUELL, liter=12.0,
    )

    (lauf,) = gruppiere_giess_laeufe([e])

    assert lauf["dauer_gesamt_s"] == 600
    assert lauf["liter_gesamt"] == 12.0
    assert lauf["laeuft_noch"] is False
    assert lauf["ende"] is not None


def test_t0566_echtes_offenes_ventil_bleibt_laufend():
    """Gegenprobe: ein OEFFNEN mit dauer=0 ist weiterhin ein laufender Puls.

    Ohne diesen Fall koennte man jedes pendantlose OEFFNEN als fertig
    ausweisen -- dann verschwaende ein wirklich offenes Ventil aus der
    Anzeige.
    """
    e = VentilEreignis(
        zeitstempel=datetime(2026, 9, 1, 9, 0),
        zone_id="bambuswald", ventil_id="dswc-1:2",
    ...
    assert lauf["laeuft_noch"] is True
    assert lauf["dauer_gesamt_s"] is None
