...
def test_ml_vorhersage_reicht_top_feature_skala_durch(tmp_path):
    ...
    # T-0476: Frischefenster an Entscheidungs-Horizont gekoppelt -- der
    # Cache-Fallback ist zusaetzlich auf MAX_FALLBACK_ALTER_STUNDEN (48 h)
    # gekappt. Ein fester Kalender-Zeitstempel waere hier ab sofort "zu alt"
    # und der Endpoint antwortete mit "Keine aktuellen Sensordaten"; der Test
    # prueft aber die Skalen-Durchreichung, nicht die Staleness.
    verarbeiter.hole_letzten_wert.return_value = SensorMessung(
        zeitstempel=datetime.now() - timedelta(minutes=5),
        zone_id="bambuswald",
        boden_feuchte=80.0,
