...
# T-0544: Richtungspruefung fuer die Plateau-Erkennung.
#
# Der bisherige Test kannte nur `max - min` und konnte deshalb einen monotonen
# ANSTIEG nicht von symmetrischem Schwanken gleicher Amplitude unterscheiden --
# fuer ein Plateau ist das aber der ganze Unterschied.
#
# Realfall, der das ausgeloest hat (kasten_4, 20.08.2026, FYTA):
#
#     Fenster 07:00-19:00, n=48, min 75, max 81  -> Spanne 6
#     Alt-Test `> 2 * plateau_max_delta` = `> 6`  -> bestanden, mit null Marge
#     Der Verlauf stieg MONOTON und lief danach auf 81 weiter.
#
# Der gespeicherte Median 79 war damit die Momentaufnahme eines laufenden
# Anstiegs, kein Gleichgewicht.
#
# **Das Kriterium ist EINE Bedingung, und das ist gemessen, nicht gesetzt.**
# Ein erster Entwurf verlangte zusaetzlich, dass die Drift einen Grossteil der
# Spannweite ausmacht. Am echten Verlauf oben liefert das Drift 3,0 gegen
# Spanne 6,0, also exakt die Haelfte -- die Zusatzbedingung haette wieder auf
# einem Gleichstand entschieden, also genau dem Fehler, den dieser Task
# behebt. Sie ist ausserdem redundant: die Spannweiten-Pruefung davor laesst
# nur Fenster bis 2*plateau_max_delta durch, in denen eine grosse Drift gar
# nicht auftreten kann.
TREND_DRIFT_MASSSTAB = "aufloesung"  # Doku-Anker; Schwelle ist die Quell-Aufloesung


def _netto_drift(werte: list[float]) -> float:
    """Gerichtete Bewegung ueber das Fenster, robust gegen Ausreisser.

    Median der ersten gegen Median der letzten Haelfte statt erstem gegen
    letztem Wert -- ein einzelner Ausreisser am Rand soll kein Plateau
    kippen. Vorzeichen bleibt erhalten (positiv = Anstieg).

    `werte` muss chronologisch sortiert sein.
    """
    import statistics
    if len(werte) < 2:
        return 0.0
    haelfte = len(werte) // 2
    return statistics.median(werte[-haelfte:]) - statistics.median(werte[:haelfte])


def _ist_gerichtet(werte: list[float], aufloesung: float) -> bool:
    """Traegt das Fenster einen systematischen Trend statt zu schwanken?

    Kriterium: die gerichtete Bewegung ist GROESSER als ein
    Quantisierungsschritt der Quelle. Ein einzelner Schritt ist auf einem
    5-pp-Raster die kleinste darstellbare Bewegung und kein Trend
    ([[fehlerpattern_detektor_unter_sensoraufloesung]]); mehr als ein Schritt
    kann Quantisierung nicht mehr erklaeren.

    Gemessen an fuenf Faellen (echter kasten_4-Verlauf plus vier Kontrollen):

        Fall                                  Spanne  Drift  Urteil
        kasten_4 20.08. (FYTA, Anstieg)          6,0    3,0  gerichtet
        flach mit Rauschen (FYTA)                2,0    0,5  Plateau
        Gardena, eine 5-pp-Stufe                 5,0    5,0  Plateau
        Gardena, Pendeln um die Bin-Grenze       5,0    0,0  Plateau
        starker Abfall                           9,4   -4,8  (faellt schon
                                                              an der Spanne)
    """
    return abs(_netto_drift(werte)) > aufloesung
...
class KalibrationsJob:
    ...
    async def _scan_alle_zonen(self, jetzt: datetime) -> None:
        ...
        # T-0484 Teil 2: frueher `z.ventil_kanal is not None`. Das Gate war
        # technisch (hat die Zone ein Ventil?), die Frage ist aber fachlich:
        # **erreicht Regen den Wurzelballen?** Nur dann darf man aus einem
        # Regen-Peak auf Feldkapazitaet schliessen. Bei einem ueberdachten
        # Balkontopf kam kein Wasser an -- der Sensorwert 12-24 h nach dem
        # Regen ist dort schlicht der Trockenwert, und ihn als
        # Feldkapazitaet zu speichern setzt eine Schwelle auf eine erfundene
        # Beobachtung.
        #
        # Andres Auskunft 05.08.: von den neun Berliner Topf-Zonen bekommt
        # **nur kasten_4** durchdringenden Regen, "aber nur, wenn der Wind
        # richtig steht". Die uebrigen acht sind in `default.yaml` deshalb
        # auf `regen_erreicht_ballen: false` gesetzt.
        zonen = [z for z in self._konfig.zonen if z.regen_erreicht_ballen]
        if not zonen:
            return
        ...
        # den aktuellen Regen-Event nicht erst nach Archiv-Latenz (3-5 Tage)
        # sieht.
    ...
    async def _scan_feldkapazitaet(
        ...
        for peak_ende, summe_mm in regen_peaks:
            ...
            if (max(werte) - min(werte)) > 2 * self._kal.plateau_max_delta:
                continue
            # T-0544: die Spannweite allein unterscheidet einen monotonen
            # Anstieg nicht von symmetrischem Schwanken. Ein Fenster, dessen
            # Bewegung fast vollstaendig in eine Richtung geht, ist kein
            # Gleichgewicht, sondern ein Verlauf -- der Median daraus waere
            # eine Momentaufnahme.
            aufloesung = await self._aufloesung_im_fenster(
                zone_id, plateau_von, plateau_bis,
            )
            if _ist_gerichtet(werte, aufloesung):
                logger.info(
                    "kalibrierung.plateau_verworfen_trend",
                    zone_id=zone_id, n=len(werte),
                    spanne=round(max(werte) - min(werte), 1),
                    drift=round(_netto_drift(werte), 1),
    ...
    async def _aufloesung_im_fenster(
        self, zone_id: str, von: datetime, bis: datetime,
    ) -> float:
        """Anzeige-Aufloesung der Sensoren, die dieses Fenster gefuellt haben.

        Aus den MESSWERTEN abgeleitet, nicht aus einer Konfig-Zuordnung: die
        Zeilen tragen ihre Quelle ohnehin, damit gibt es keine zweite
        Wahrheit. Zonen mit Sensoren beider Bauart (hecke hat einen Gardena
        UND einen FYTA) bekommen die GROEBERE Annahme -- der Trend-Test soll
        im Zweifel schweigen, nicht verwerfen.
        """
        messungen = await self._speicher.hole_messungen(zone_id, von, bis)
        quellen = {
            m.quelle for m in messungen if getattr(m, "quelle", None)
        }
        if not quellen:
            return AUFLOESUNG_PP_DEFAULT
        return max(aufloesung_pp(q) for q in quellen)
    ...
    async def _scan_wirkungsrate(
        ...
        schliessen_events = [
            ...
            # T-0455: war `(MANUELL, AUTOMATIK)` als Literal. Seit es einen
            # eigenen `zeitplan`-Ausloeser gibt, waeren Cloud-Zeitplan-Laeufe
            # hier still aus dem Fit gefallen, obwohl sie echtes Kanal-Wasser
            # mit echter Dauer sind (und vor T-0455 als manuell/automatik
            # drin waren). Set statt Literal -> naechster Wert faellt nicht
            # wieder stumm heraus.
