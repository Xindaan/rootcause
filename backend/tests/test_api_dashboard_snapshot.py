...
def test_t0495_muellwerte_im_fenster_erzeugen_keinen_zweiten_cache_eintrag(
    client_mit_daten,
):
    """T-0495 (Audit B1): der Schluessel kam aus dem ROHEN Query-String.

    "24h", "24h,quatsch" und "24h,,24h" meinen dasselbe -- die Validierung
    wirft die Muellwerte ohnehin weg --, erzeugten aber je einen eigenen
    Eintrag mit einer vollstaendigen Antwort darin. Bei 9,9 MB fuer einen
    Langsnapshot ist das der Hebel, nicht die Zahl der Eintraege.
    """
    from bewaesserung.api_server import _SNAPSHOT_CACHE
    client, _, _ = client_mit_daten
    _SNAPSHOT_CACHE.clear()

    a = client.get("/api/dashboard-snapshot?fenster=24h").json()
    for muell in ("24h,quatsch", "24h,,24h", " 24h ", "24h,7dX"):
        b = client.get(f"/api/dashboard-snapshot?fenster={muell}").json()
        assert b["fenster"] == ["24h"]
        # Cache-Treffer: identischer Compute-Zeitstempel.
        assert b["zeitstempel"] == a["zeitstempel"], muell

    assert len(_SNAPSHOT_CACHE) == 1, (
        f"ein semantischer Schluessel, aber {len(_SNAPSHOT_CACHE)} Eintraege"
    )


def test_t0495_cache_bleibt_beschraenkt_und_raeumt_abgelaufene_weg():
    """T-0495: das Dict war unbegrenzt und wurde nur bei `konfiguriere_api`
    geleert -- praktisch beim Service-Restart. Abgelaufene Eintraege blieben
    resident, obwohl sie nie wieder ausgeliefert werden.

    Getestet wird der Schreibpfad direkt, weil ueber HTTP nur die vom
    Frontend erzeugbaren Schluessel entstehen -- die Obergrenze ist aber
    genau der Backstop fuer den Fall, dass das nicht mehr stimmt.
    """
    import time as _time

    from bewaesserung import api_server

    api_server._SNAPSHOT_CACHE.clear()
    try:
        for i in range(api_server._SNAPSHOT_CACHE_MAX + 10):
            api_server._snapshot_cache_setze((f"z{i}", ("24h",), False), {"i": i})
        assert len(api_server._SNAPSHOT_CACHE) == api_server._SNAPSHOT_CACHE_MAX

        # Der aelteste ist weg, der juengste da.
        assert ("z0", ("24h",), False) not in api_server._SNAPSHOT_CACHE
        letzter = (f"z{api_server._SNAPSHOT_CACHE_MAX + 9}", ("24h",), False)
        assert letzter in api_server._SNAPSHOT_CACHE

        # Abgelaufene fliegen beim naechsten Schreiben raus, nicht erst beim
        # Restart.
        api_server._SNAPSHOT_CACHE.clear()
        alt = _time.monotonic() - api_server._SNAPSHOT_CACHE_TTL_S - 1
        api_server._SNAPSHOT_CACHE[("alt", ("24h",), False)] = (alt, {})
        api_server._snapshot_cache_setze(("neu", ("24h",), False), {})
        assert ("alt", ("24h",), False) not in api_server._SNAPSHOT_CACHE
        assert ("neu", ("24h",), False) in api_server._SNAPSHOT_CACHE
    finally:
        api_server._SNAPSHOT_CACHE.clear()
