...
class _MotorStubT0256:
    ...
    def __init__(
        self, empfehlungen: dict[str, GiessEmpfehlung],
        aktive_kanal_zonen: set[str] | None = None,
    ):
        self._empf = empfehlungen
        self.aufrufe: list[str] = []
        # T-0443: Zonen, auf deren Kanal gerade Wasser laeuft.
        self._aktiv = aktive_kanal_zonen or set()
    ...
    async def kanal_aktiv_bewaesserung(
        self, zone_id: str, jetzt=None,
    ) -> bool:
        return zone_id in self._aktiv
...
def test_t0535_push_meldet_die_testdosis_statt_der_berechneten(speicher):
    """T-0535: bei scharfem Dosis-Test faehrt das Ventil die Teststufe.

    Der Push ging bis 11.08.2026 immer von `dauer_s_empfehlung` aus. Waehrend
    eines Dosis-Tests haette Andre damit auf dem Handy eine andere Zahl
    gelesen als im Dashboard und als real gelaufen ist.
    """
    zone = _baue_zone_mit_plateau("bambus")
    empf = _baue_empfehlung_gieße(zone.zone_id, dauer_s=480)
    # Berechnet waeren 8 min, gefahren wird die Teststufe 70 min.
    empf = empf.model_copy(update={"dauer_s_dosis_test": 4200})
    motor = _MotorStubT0256({zone.zone_id: empf})
    benach = _BenachrichtigerStub()
    jetzt = datetime(2026, 5, 26, 9, 30)
    _run(_flute_sensor(speicher, zone.zone_id, jetzt))
    ...
    assert "70 min" in shadow_pushes[0], shadow_pushes[0]
    assert "8 min" not in shadow_pushes[0]
...
def test_t0443_kein_shadow_push_waehrend_wasser_laeuft(speicher):
    """T-0443: kein Push, solange auf dem Kanal der Zone Wasser laeuft.

    Realfall 28.07. 05:14: "Shadow-Empfehlung Bambuswald: giesse 90 min",
    waehrend der manuelle Hauptlauf seit 04:32 offen war. Der scharfe
    Auto-Loop ueberspringt aktive Kanaele bereits (main.py); der Shadow-Push
    meldete etwas, was das Ventil nachweislich nicht getan haette.
    """
    zone = _baue_zone_mit_plateau("bambus")
    motor = _MotorStubT0256(
        {zone.zone_id: _baue_empfehlung_gieße(zone.zone_id, dauer_s=480)},
        aktive_kanal_zonen={zone.zone_id},
    )
    benach = _BenachrichtigerStub()
    jetzt = datetime(2026, 5, 26, 9, 30)
    _run(_flute_sensor(speicher, zone.zone_id, jetzt))
    ...
    shadow_pushes = [t for _, t in benach.gesendet if "Shadow-Empfehlung" in t]
    assert shadow_pushes == [], (
        f"kein Push waehrend laufender Bewaesserung erwartet: {benach.gesendet}"
    )
    # Der Motor wird gar nicht erst befragt -- der Check sitzt davor.
    assert motor.aufrufe == []
    # Kein Throttle-Eintrag: der naechste faellige Tick nach Lauf-Ende soll
    # normal pushen duerfen, statt durch min_pause gesperrt zu sein.
    letzter = _run(speicher.hole_letzten_watchdog_push(
        TYP_SHADOW_EMPFEHLUNG, zone.zone_id,
    ))
    assert letzter is None


def test_t0443_race_fallback_pusht_nicht_in_laufendes_wasser(speicher):
    """Der Grund, warum der Check auf der PUSH-Ebene sitzen muss und nicht
    nur im Kritisch-Bypass des Motors: der T-0261-Race-Fallback setzt
    `soll=True` an der Motor-Entscheidung VORBEI (er liest einen aelteren
    empfehlungs_audit-Snapshot). Ein Fix nur im Bypass laesst diesen Pfad
    offen -- eine Zone, die zum ersten Mal pusht, pusht sonst weiter in
    laufendes Wasser hinein.

    Identischer Aufbau wie `test_t0261_race_fix_...`, nur mit aktivem Kanal.
    """
    zone = _baue_zone_mit_plateau("bambus_yogaraum")
    motor = _MotorStubT0256(
        {zone.zone_id: _baue_empfehlung_pause_aktiv_blockiert(zone.zone_id)},
    ...
    shadow_pushes = [t for _, t in benach.gesendet if "Shadow-Empfehlung" in t]
    assert shadow_pushes == [], (
        f"Race-Fallback darf nicht in laufendes Wasser pushen: "
        f"{benach.gesendet}"
    )


def test_t0443_alter_motor_stub_ohne_kanal_check_pusht_weiter(speicher):
    """Rueckwaerts-Kompatibilitaet: ein Motor ohne
    `kanal_aktiv_bewaesserung` (alte Stubs, alte Mocks) darf den Push nicht
    stillegen. `getattr`-Pfad im Watchdog.
    """
    zone = _baue_zone_mit_plateau("bambus")
...
# --- T-0560: `wohlfuehl_grenze` ist kein Push-Grund ---

def _baue_empfehlung_wohlfuehl(zone_id: str, dauer_s: int = 1320) -> GiessEmpfehlung:
    """Panel-Empfehlung mit `soll_bewaessern=True`, aber sanftem Typ.

    Genau die Kombination, die `vorhersage_zone` liefert: dort ist
    `soll_bewaessern` bewusst `empf_typ != "kein_bedarf"` und schliesst
    `wohlfuehl_grenze` ein.
    """
    empf = _baue_empfehlung_gieße(zone_id, dauer_s=dauer_s)
    return empf.model_copy(update={"empfehlungs_typ": "wohlfuehl_grenze"})


def test_t0560_wohlfuehl_grenze_loest_keinen_shadow_push(speicher):
    """Ein Wohlfuehl-Hinweis darf keine iMessage werden.

    `entscheidung_pro_zone.EMPFEHLUNG_TRIGGERT_BEWAESSERUNG` fuehrt
    `wohlfuehl_grenze` ausdruecklich NICHT als Trigger ("nur Hinweis im
    Dashboard"). Der Watchdog gatete aber allein auf `soll_bewaessern` --
    aus dem Hinweis wurde ein "giesse 22 min"-Push, waehrend die Engine
    nichts tat.
    """
    zone = _baue_zone_mit_plateau("bambus")
    motor = _MotorStubT0256({zone.zone_id: _baue_empfehlung_wohlfuehl(zone.zone_id)})
    benach = _BenachrichtigerStub()
    jetzt = datetime(2026, 5, 26, 9, 30)
    _run(_flute_sensor(speicher, zone.zone_id, jetzt))
    ...
    shadow_pushes = [t for _, t in benach.gesendet if "Shadow-Empfehlung" in t]
    assert shadow_pushes == [], (
        f"wohlfuehl_grenze darf nicht pushen, gesendet: {shadow_pushes}"
    )


def test_t0563_defekter_trigger_legt_die_anderen_nicht_lahm(speicher):
    """Ein dauerhaft werfender Trigger darf die uebrigen nicht verdecken.

    Die sieben Pruefungen liefen ohne eigenes `except` hintereinander, und
    `_letzte_aktualisierung` war oben schon gesetzt. Ein Fehler in Trigger A
    machte B bis H unsichtbar -- darunter den Husqvarna-Soft-Ban-Alarm.
    """
    zone = _baue_zone_mit_plateau("bambus")
    motor = _MotorStubT0256({zone.zone_id: _baue_empfehlung_gieße(zone.zone_id)})
    benach = _BenachrichtigerStub()
    ...
    async def _wirft(_jetzt):
        raise RuntimeError("Trigger A kaputt")

    job._pruefe_akut_in_folge = _wirft  # type: ignore[assignment]

    stat = _run(job.pruefe_und_sende_wenn_faellig(jetzt=jetzt))

    assert stat["geprueft"] == 1, "der Tick darf nicht durchschlagen"
    shadow_pushes = [t for _, t in benach.gesendet if "Shadow-Empfehlung" in t]
    assert len(shadow_pushes) == 1, (
        "die nachgelagerten Trigger muessen weiterlaufen, "
        f"gesendet: {benach.gesendet}"
    )
