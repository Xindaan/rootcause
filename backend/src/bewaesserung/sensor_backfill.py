...
der User klassifiziert manuell via Ops-Tab (B2).
...
class SensorBackfillJob:
    ...
    async def _pruefe_zone(self, zone_id: str, jetzt: datetime) -> int:
        ...
        for sensor_id, sensor_messungen in nach_sensor.items():
            ...
            for i in range(1, len(sensor_messungen)):
                ...
                if await self._schreibe_wenn_kandidat(zone_id, alt, neu):
                    geschrieben += 1
        ...
        if zone_id in self._rollup_pro_zone:
            for sensor_messungen in nach_sensor.values():
                if len(sensor_messungen) >= 2:
                    geschrieben += await self._scan_rollup(zone_id, sensor_messungen)
        return geschrieben
    ...
    async def _schreibe_wenn_kandidat(
        ...
        return await self._versuche_schreiben(zone_id, alt, neu, delta)

    async def _scan_rollup(
        self, zone_id: str, messungen: list[SensorMessung],
    ) -> int:
    ...
    async def _versuche_schreiben(
        ...
        if await self._aktive_bewaesserung_auf_kanal(zone_id):
            logger.debug(
        ...
        # T-0312: Cross-Spray-Guard. An dieser Stelle ist KEIN eigener
        ...
        # Strahl auf den Sensor -- KEIN eigener Microdrip-Lauf. Nicht als
        # Bewaesserung schreiben (sonst verfaelscht es Wirkungsrate/ML/Schwellen).
        if await self._ist_cross_spray(zone_id, start, stop):
            return False
    ...
    async def _aktive_bewaesserung_auf_kanal(self, zone_id: str) -> bool:
        """T-0123: True wenn auf dem Kanal der Zone gerade eine Backend-
        Bewaesserung laeuft (live_lauf_state). Faellt auf False zurueck wenn
        Mapping fehlt, Speicher die Methode nicht hat, oder etwas crasht.
        """
        ...
        return any(
            s.get("kanal") == kanal
            and (geraet_id is None or s.get("geraet_id") == geraet_id)
            for s in states
        )

    async def _ist_cross_spray(
        self, zone_id: str, start: datetime, stop: datetime,
    ) -> bool:
        """T-0312: True, wenn der Feuchte-Sprung wahrscheinlich Cross-Spray
        einer Nachbar-Zone ist (deren Sprinkler-Strahl den Sensor dieser Zone
        trifft) statt eines eigenen Kanal-Laufs.
        ...
        for quell_zone in quellen:
            ...
            if any(e.ventil_id != "sensor_heuristik" for e in events):
                logger.info(
                    "sensor_backfill.skip_cross_spray",
                    zone_id=zone_id, quell_zone=quell_zone,
                    start=start.isoformat(timespec="minutes"),
                )
                return True
    ...
    async def _regen_im_fenster(
        ...
        per Stunde mischt: Archiv hat Vorrang (ERA5-Ground-Truth), fehlende
        ...
        **Regression-Fix 2026-04-22**: Vorher war die Logik `if archiv: nur
        ...
        keine fuer den Regen-Zeitraum ab 15:00 (ERA5-Latenz 3-5 Tage). Die
