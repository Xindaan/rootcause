...
@pytest.mark.asyncio
async def test_t0493_doppelte_messung_wird_still_verworfen(tmp_path):
    """T-0493: gleiche (zone, geraet, zeitstempel) darf nur einmal existieren.

    Der 2-s-Dedupcheck in `speichere_messung` sucht `zeitstempel < neuer` und
    sieht eine Zeile mit exakt gleichem Timestamp nicht; ausserdem sind SELECT
    und INSERT nicht atomar. Gemessen am 05.08.: 646 Gruppen mit 1519
    Zusatzzeilen, hoechste Multiplizitaet 79. Die Nutzwerte sind identisch --
    keine falsche Anzeige, aber das ML-Training gewichtet eine 79-fach
    vorhandene Messung 79-fach.

    `INSERT OR IGNORE` plus UNIQUE-Index: ein Duplikat ist ein No-op, keine
    Exception. Eine Exception wuerde Live-Pfade toeten, die stillschweigend
    auf Erfolg bauen (T-0408-Lehre).
    """
    s = Speicher(str(tmp_path / "dedupe.db"))
    await s.verbinden()
    try:
        ts = datetime(2026, 8, 5, 12, 0, 0)
        for _ in range(3):
            await s.speichere_messung(SensorMessung(
                zone_id="z1", geraet_id="g1", zeitstempel=ts,
                boden_feuchte=42.0, quelle=DatenQuelle.GARDENA,
            ))
        cursor = await s._db.execute(
            "SELECT COUNT(*) FROM sensor_messung WHERE zone_id='z1'",
        )
        (anzahl,) = await cursor.fetchone()
        assert anzahl == 1, f"{anzahl} Zeilen statt 1 -- Duplikate kommen durch"

        # Gegenprobe: ein anderer Zeitstempel derselben Zone bleibt erhalten.
        await s.speichere_messung(SensorMessung(
            zone_id="z1", geraet_id="g1", zeitstempel=ts + timedelta(minutes=5),
            boden_feuchte=43.0, quelle=DatenQuelle.GARDENA,
        ))
        cursor = await s._db.execute(
            "SELECT COUNT(*) FROM sensor_messung WHERE zone_id='z1'",
        )
        (anzahl2,) = await cursor.fetchone()
        assert anzahl2 == 2, "der Index darf echte neue Messungen nicht schlucken"
    finally:
        await s.schliessen()
