...
4. **Regime-Stempel (T-0349)**: klassifiziert Snapshot-Fenster
   ...
   (Archiv-Lag ~5 Tage) das jeweilige Fenster abdeckt. Bewusst vom Eval
...
class EmpfehlungsAuditJob:
            ...
            for zeile in zeilen:
                ...
                for horizont, spalte in ((6, "regime_6h"), (24, "regime_24h")):
                    ...
                    if (
                        ...
                        continue  # Archiv-Lag — naechster Tick.
    ...
    async def _sensor_nah_zeitpunkt(
        ...
        T-0561: bei Zonen mit `aggregat_lead_geraet` NUR diesen Sensor. Die
        Prognose stammt vom Lead; sie gegen einen anderen Sensor derselben
        Zone zu evaluieren misst den Skalenunterschied, nicht den
        Prognosefehler. Und die anderen gewinnen fast immer, weil FYTA
        haeufiger sendet als Gardena.

        An den letzten 200 faelligen Audit-Zeilen nachgezaehlt (Live-DB,
        04.09.): waldblumenhain evaluierte 173 davon ueber einen FREMDEN
        Sensor, hecke 163. Mit Lead-Filter bleiben 194 bzw. 193 von 200
        auswertbar -- die Abdeckung kostet also rund drei Prozent, waehrend
        rund 85 Prozent falscher Bezugswerte verschwinden.

        Findet der Lead im Fenster nichts, bleibt es bei None (Zeile wird
        spaeter erneut geprueft). Das folgt T-0384: die Sensor-IDENTITAET
        wechselt nie still.
        """
        ...
        messungen = await self._speicher.hole_messungen(
            zone_id, von=von, bis=bis, geraet_id=lead,
        )
        if not messungen:
            return None
