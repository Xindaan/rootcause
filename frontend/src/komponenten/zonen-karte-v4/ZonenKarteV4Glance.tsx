...
function pruefeStale(isoStr: string | null | undefined): {
  ...
  // T-0476: bewusst STRENGER als das Backend-Abruf-Fenster (240 min).
  // Das Backend liefert Werte bis 4 h Alter und giesst auch auf ihnen;
  // der Marker ab 2 h traegt die Ehrlichkeit der Anzeige (Entscheid
  // 10.08.2026). Keine dritte Verwendbarkeits-Wahrheit, sondern eine
  // Anzeige-Warnstufe unterhalb der Verwendbarkeits-Grenze.
...
export function ZonenKarteV4Glance({
  ...
  // T-0573 AK6: Gegenfall zur unterdrueckten Prognose. Ist der Wert aus
  // dem 240-min-Abruf-Fenster gefallen, stand hier bisher nur "-" -- die
  // Karte verschwieg einen Messwert, den sie hat (pilea/fuchsie/avocado,
  // Bluetooth-only-FYTA mit schubweisem Sync, T-0214). Jetzt steht die
  // Zahl da, gedaempft und mit ihrem Alter. Eine Empfehlung leitet sich
  // daraus weiterhin NICHT ab -- `ist` bleibt null.
  const rueckfall = ist == null && zone.letzter_bekannter_wert != null
    ? zone.letzter_bekannter_wert
    : null
  const rueckfallAlter = zone.letzter_bekannter_alter_h ?? null
  ...
  // T-0573 AK6: faellt der Wert aus dem Abruf-Fenster, ist
  // `letztes_update` null und die Zeile sagte 'kein Update' -- obwohl
  // der Zeitpunkt bekannt ist. Dann den Zeitstempel des letzten
  // bekannten Werts nehmen, damit dort 'vor 10 h' steht.
  const { stale, tooltip: updateTooltip, kurzLabel: updateKurz } =
    pruefeStale(zone.letztes_update ?? zone.letzter_bekannter_zeit)
  // Q19: nur der 24h-ML-Wert als Bar-Suffix. 6h/12h + q10-q90-Band
  // stehen im Detail-Chart -- ersetzt die drei Header-Werte (MlKompakt).
  // T-0573: die Zahl nur zeigen, wenn das Backend sie fuer belastbar
  // haelt. Vorher stand hier direkt `feuchte_prognose` -- deshalb konnte
  // am 09.09. "24h: 66" neben einem Istwert von 30 stehen: korrekt
  // gerechnet, aber auf einer 25 h alten Zeile eines inzwischen
  // ausgebauten Sensors.
  const ml24hRoh = mlVorhersage?.['24h'] ?? null
  const mlGueltig = ml24hRoh?.gueltig !== false
  const ml24h = mlGueltig ? (ml24hRoh?.feuchte_prognose ?? null) : null
  const mlUngueltigLabel = ml24hRoh != null && !mlGueltig
    ? prognoseUngueltigLabel(ml24hRoh)
    : null
  ...
  return (
    <div className="zk-glance">
        ...
        <div className="zk-head__r">
          <span
            ...
            title={ist != null
              ...
              : rueckfall != null
                ? `Letzter bekannter Wert: ${Math.round(rueckfall)}%`
                  + (rueckfallAlter != null ? `, gemessen vor ${rueckfallAlter.toFixed(1)} h` : '')
                  + '.\nZu alt fuer eine Empfehlung (Abruf-Fenster 4 h), aber vorhanden -- '
                  + 'deshalb wird er gezeigt statt verschwiegen.'
                : updateTooltip}
          >
            {ist != null
              ? Math.round(ist)
              : rueckfall != null
                ? <span className="zk-head__pct--rueckfall">{Math.round(rueckfall)}</span>
                : '-'}
            {(stale || rueckfall != null) && (
              <span className="zk-head__stale-mark" aria-label="veraltete Messung"> ⚠</span>
            )}
          </span>
      ...
      <div className="zk-bar-row">
        ...
        {/* T-0573: keine stille Luecke. Dass die Prognose fehlt, ist eine
            Aussage -- sonst sieht die Karte aus wie eine ohne ML-Modell. */}
        {mlUngueltigLabel != null && (
          <span className="zk-ml-suffix zk-ml-suffix--ungueltig" title={mlUngueltigLabel.tooltip}>
            <span className="zk-ml-suffix__lbl">24h:</span>
            <b>{mlUngueltigLabel.kurz}</b>
          </span>
        )}
      </div>
    </div>
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ left: `${p.schwelleMin}%`, right: `${p.schwelleMax != null ? 100 - p.schwelleMax : 0}%` }}
