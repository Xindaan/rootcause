...
# --- T-0561: Evaluation gegen den Lead-Sensor ---

def _msg_geraet(zeit: datetime, feuchte: float, geraet: str, zone: str = "waldblumenhain"):
    return SensorMessung(
        zeitstempel=zeit, zone_id=zone, geraet_id=geraet,
        boden_feuchte=feuchte, boden_temperatur=15.0, batterie_prozent=95.0,
    )


def _lege_snapshot(speicher, zone: str, zeit: datetime, prog_6h: float):
    _run(speicher.setze_empfehlungs_audit(
        zeitstempel=zeit, zone_id=zone,
        empfehlungs_typ="praeventiv", soll_bewaessern=False,
        blocker_typ="ZEITFENSTER", feuchte_aktuell=50.0,
        welkepunkt_wert=32.0, optimum_min=40.0, optimum_max=60.0,
        prognose_quelle="ml", prognose_6h=prog_6h, prognose_12h=45.0,
        prognose_24h=42.0, tage_bis_welkepunkt=2.5,
        dauer_s_empfehlung=1800, aktive_strategie="haeufig_klein",
    ))


def test_t0561_eval_nimmt_den_lead_sensor_nicht_den_naechsten(speicher):
    """Die Prognose stammt vom Lead -- also muss der Ist-Wert es auch.

    Der Nachbarsensor liegt zeitlich naeher am Ziel und haette deshalb
    gewonnen. Er liegt aber in einem anderen Skalenraum: an der Live-DB
    gemessen trennen Gardena- und FYTA-Median bei waldblumenhain rund
    30 pp. Die gemessene `abweichung_6h` waere dann der Skalenunterschied,
    nicht der Prognosefehler -- und genau darauf schauen Drift-Ampel und
    Shadow-Vergleich.
    """
    speicher.setze_aggregat_lead({"waldblumenhain": "gardena-lead"})
    motor = _MotorStub({"waldblumenhain": _empfehlung("waldblumenhain")})
    job = EmpfehlungsAuditJob(
        speicher, motor, zone_ids=["waldblumenhain"], intervall_minuten=60,
    )
    snapshot_zeit = datetime.now() - timedelta(hours=7)
    _lege_snapshot(speicher, "waldblumenhain", snapshot_zeit, prog_6h=48.0)

    ziel = snapshot_zeit + timedelta(hours=6)
    # Fremder Sensor exakt am Ziel, Lead 20 min daneben -> ohne Filter
    # gewinnt der fremde.
    _run(speicher.speichere_messung(_msg_geraet(ziel, 15.0, "fyta-a")))
    _run(speicher.speichere_messung(
        _msg_geraet(ziel + timedelta(minutes=20), 47.0, "gardena-lead"),
    ))

    _run(job.aktualisiere_wenn_faellig())

    evaluierte = [
        e for e in _run(speicher.hole_empfehlungs_audit(tage=1))
        if e["evaluiert_am"] is not None
    ]
    assert len(evaluierte) == 1
    assert evaluierte[0]["ist_feuchte_6h"] == 47.0, (
        "Ist-Wert muss vom Lead stammen, nicht vom naeheren Fremdsensor"
    )
    assert evaluierte[0]["abweichung_6h"] == pytest.approx(1.0, rel=0.01)


def test_t0561_ohne_lead_bleibt_das_alte_verhalten(speicher):
    """Gegenprobe: Zonen ohne `aggregat_lead_geraet` sind unveraendert.

    Ohne diesen Fall koennte der Filter versehentlich auf alle Zonen
    wirken und Einzelsensor-Zonen die Auswertung nehmen.
    """
    motor = _MotorStub({"bambuswald": _empfehlung()})
    job = EmpfehlungsAuditJob(
        speicher, motor, zone_ids=["bambuswald"], intervall_minuten=60,
    ...
    snapshot_zeit = datetime.now() - timedelta(hours=7)
    _lege_snapshot(speicher, "bambuswald", snapshot_zeit, prog_6h=48.0)
    _run(speicher.speichere_messung(_msg(
        snapshot_zeit + timedelta(hours=6, minutes=10), 47.0,
    )))
