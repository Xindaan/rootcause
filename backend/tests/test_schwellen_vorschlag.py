...
def test_t0491_zone_ohne_nass_obergrenze_wirft_nicht(speicher):
    """T-0491: `feuchte_schwelle_max=None` darf den Endpoint nicht killen.

    Realfall 13.08.2026: nachdem zitrus und mandevilla_maxi ihre dauerhaft
    anstehende Nass-Warnung abgegeben hatten, warf
    `/api/schwellen-vorschlag` einen ValidationError und lieferte 500 --
    sichtbar erst in der Browser-Konsole, nicht in der Suite.

    Der Isomorphie-Check hatte auf das FELD `feuchte_schwelle_max` gegrept
    und diesen Aufrufer uebersehen, weil er ueber die Accessor-Funktion
    `effektiv_schwelle_max()` geht. Deshalb dieser Test: er deckt die
    Klasse an der Stelle ab, an der sie zuschlaegt.
    """
    zonen = [
        ZonenKonfig(zone_id="zitrus", name="Zitrus",
                    feuchte_schwelle_min=22, feuchte_schwelle_max=None),
        ZonenKonfig(zone_id="rasen", name="Rasen",
                    feuchte_schwelle_min=40, feuchte_schwelle_max=75),
    ]
    jetzt = datetime(2026, 4, 20, 12, 0)
    ergebnisse = _run(berechne_vorschlaege_fuer_alle(speicher, zonen, jetzt=jetzt))
    assert len(ergebnisse) == 2
    ohne = next(e for e in ergebnisse if e.zone_id == "zitrus")
    assert ohne.max_aktuell is None, "None muss durchgereicht werden, nicht 0"
    mit = next(e for e in ergebnisse if e.zone_id == "rasen")
    assert mit.max_aktuell == 75


# --- T-0561: Perzentil nur ueber den Lead-Sensor ---

def test_t0561_vorschlag_nutzt_nur_den_lead_sensor(speicher):
    """Der Vorschlag wird gegen die Schwelle des Lead-Sensors gestellt.

    Ohne Filter mischt das Perzentil zwei Skalenraeume. An der Live-DB
    gemessen (30 Tage): waldblumenhain p10 gemischt 12,0 gegen 40,0 nur
    Lead. Uebernommen haette der User die Schwelle um rund 15 pp gesenkt --
    auf Sand die teure Fehlerrichtung.
    """
    zonen = [ZonenKonfig(
        zone_id="waldblumenhain", name="Waldblumen",
        feuchte_schwelle_min=40, feuchte_schwelle_max=70,
    )]
    jetzt = datetime(2026, 4, 20, 12, 0)
    speicher.setze_aggregat_lead({"waldblumenhain": "gardena-lead"})
    # Lead streut 30-79 (breit genug fuer die Spannen-Pruefung), der
    # Fremdsensor liegt konstant tief bei 10.
    for i in range(300):
        _run(speicher.speichere_messung(SensorMessung(
            zeitstempel=jetzt - timedelta(minutes=i * 10),
            zone_id="waldblumenhain", geraet_id="gardena-lead",
            boden_feuchte=30.0 + (i % 50), boden_temperatur=15.0,
            quelle=DatenQuelle.GARDENA,
        )))
        _run(speicher.speichere_messung(SensorMessung(
            zeitstempel=jetzt - timedelta(minutes=i * 10 + 3),
            zone_id="waldblumenhain", geraet_id="fyta-a",
            boden_feuchte=10.0, boden_temperatur=15.0,
    ...
    e = ergebnisse[0]
    assert e.n_messungen == 300, (
        f"nur die Lead-Messungen zaehlen, waren {e.n_messungen}"
    )
    assert e.basis == "berechnet"
    assert e.min_vorschlag is not None and e.min_vorschlag >= 25, (
        "mit gemischten Sensoren waere der Vorschlag Richtung 10 gerutscht"
    )
