...
function ZonenKarteV4Inner({
  ...
  useEffect(() => {
    ...
    const stoppeIntervall = starteSichtbarkeitsIntervall(() => setRefreshKey(k => k + 1), 60_000)
  ...
  const glanceZustand: 'kritisch' | 'warn' | 'info' | 'ok' | 'stumm' =
    ...
    : 'ok'
  // T-0573: Trend nur auf dem Lead-Sensor und nicht ueber eine
  // Kalibrier-Fenstergrenze hinweg -- sonst mischt er Skalen bzw.
  // zwei physische Geraete zu einer erfundenen pp/h-Rate.
  const trendPpProH = berechneTrendPpProH(trendBasis, {
    leadGeraet: zone.aggregat_lead_geraet,
    fenster: zone.ausschluss_fenster,
  })
