...
# --- T-0567: Feature-Serien pro Sensor, nicht pro Zone ---

def test_t0567_gruppierung_trennt_die_sensoren_einer_zone():
    """Lag-, Rolling- und ZIEL-Lookups suchen in EINER Liste.

    Lief die nur ueber `zone_id`, mischte eine Multi-Sensor-Zone
    Skalenraeume: an Produktivdaten gemessen stammten bei
    `waldblumenhain` 13,0 % der Labels von einem anderen Sensor als der
    Eingangswert (Median-Fehler 10 pp, max 100 pp), bei `hecke` 7,3 %
    mit Median 20 pp. Konkret bekam eine Gardena-Zeile mit eigenem Wert 45
    ein `ziel_delta_6h` von -34, weil das Ziel von einer FYTA-Sonde kam.
    """
    ...
    for i in range(4):
        messungen.append(SensorMessung(
            ...
            boden_feuchte=45.0, quelle=DatenQuelle.GARDENA,
        ))
        messungen.append(SensorMessung(
            zeitstempel=basis + timedelta(minutes=30 * i + 5),
            zone_id="waldblumenhain", geraet_id="fyta-a",
            boden_feuchte=11.0, quelle=DatenQuelle.FYTA,
        ))

    gruppen = FeatureExtraktor._gruppiere_messungen(None, messungen)

    assert set(gruppen) == {
        ("waldblumenhain", "gardena-lead"),
        ("waldblumenhain", "fyta-a"),
    }
    for schluessel, serie in gruppen.items():
        werte = {m.boden_feuchte for m in serie}
        assert len(werte) == 1, (
            f"{schluessel} mischt Skalen: {werte}"
        )
        # Chronologie bleibt erhalten.
        assert serie == sorted(serie, key=lambda m: m.zeitstempel)


def test_t0567_einzelsensor_zone_bleibt_eine_serie():
    """Gegenprobe: Zonen mit einem Sensor aendern sich nicht."""
    from bewaesserung.ml.features import FeatureExtraktor
    ...
    messungen = [
        SensorMessung(
            zeitstempel=basis + timedelta(minutes=30 * i),
            zone_id="bambuswald", geraet_id="aaaa0003",
            boden_feuchte=40.0 + i, quelle=DatenQuelle.GARDENA,
        )
        for i in range(5)
    ]
    ...
    assert list(gruppen) == [("bambuswald", "aaaa0003")]
    assert len(gruppen[("bambuswald", "aaaa0003")]) == 5
