...
- Response-Features (`ml/response_features.py`) nehmen AquaBloom-Rows
  nur bei `delta_6h > 1.0` ins Training. Gilt erst seit T-0482
  (02.08.2026) -- vorher erreichte kein Puls den Feature-Bau, weil der
  Pseudo-Kreis zwar erzeugt, aber nie als Key gebaut wurde. Pump-Zonen
  brauchen ausserdem `min_events_pump_zone` (20) statt `min_events`
  Zeilen, bevor ein Modell entsteht.
