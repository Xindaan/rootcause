...
class WatchdogJob:
    ...
    async def pruefe_und_sende_wenn_faellig(
        ...
        gesendet = 0
        gesendet += await self._pruefe_akut_in_folge(jetzt)
        gesendet += await self._pruefe_husqvarna_block(jetzt)
        gesendet += await self._pruefe_endpoint_health(jetzt)
        gesendet += await self._pruefe_pflege_erinnerungen(jetzt)
        gesendet += await self._pruefe_unbekannt_events(jetzt)
        gesendet += await self._pruefe_shadow_empfehlungen(jetzt)
    ...
    def _format_shadow_empfehlung(
        ...
        dauer_quelle = dauer_s_override if dauer_s_override is not None \
            else (empf.dauer_s_empfehlung or 0)
