...
def test_raeume_alte_archive_behaelt_juengste_n(tmp_path):
    ...
    _raeume_alte_archive(basis, behalten=5, geschuetzte_dateien=frozenset())
...
def test_swap_live_mit_tmp_raeumt_archive_auf(tmp_path):
    ...
    for i in range(1, 8):
        ...
        _swap_live_mit_tmp(
            live, tmp, archiv, archiv_behalten=5,
            geschuetzte_dateien=frozenset(),
        )
...
def test_raeume_alte_archive_behaelt_referenzierten_ordner(tmp_path):
    """T-0478: ein alter Ordner ausserhalb der juengsten 5 ueberlebt,
    wenn er eine referenzierte Modelldatei enthaelt."""
    basis = tmp_path / "hecke_archiv"
    basis.mkdir()
    namen = [f"2026-05-{10 + i:02d}_080000" for i in range(8)]
    for i, n in enumerate(namen):
        (basis / n).mkdir()
        (basis / n / f"modell_6h_q50_v{i}.lgbm").write_text("x")

    _raeume_alte_archive(
        basis, behalten=5, geschuetzte_dateien={"modell_6h_q50_v0.lgbm"},
    )

    verbleibend = sorted(p.name for p in basis.iterdir())
    assert namen[0] in verbleibend, "referenzierter Ordner wurde geloescht"
    assert verbleibend == [namen[0], *namen[3:]]


def test_raeume_alte_archive_loescht_nichts_bei_unbekannter_schutzmenge(
    tmp_path,
):
    """T-0478: `None` heisst "DB nicht lesbar" und darf NICHT als
    "nichts referenziert" durchgehen. Ein zu grosses Archiv ist
    reparabel, ein geloeschtes Modell nicht."""
    basis = tmp_path / "hecke_archiv"
    basis.mkdir()
    for i in range(8):
        (basis / f"2026-05-{10 + i:02d}_080000").mkdir()

    _raeume_alte_archive(basis, behalten=5, geschuetzte_dateien=None)

    assert len(list(basis.iterdir())) == 8, "trotz None wurde geloescht"


def test_swap_live_mit_tmp_reicht_schutzmenge_durch(tmp_path):
    """T-0478: ein Archiv-Ordner ausserhalb der juengsten 5 ueberlebt den
    Deploy-Swap, wenn er eine referenzierte Modelldatei enthaelt."""
    live = tmp_path / "hecke"
    archiv_basis = tmp_path / "hecke_archiv"
    live.mkdir()
    (live / "modell_neu.lgbm").write_text("live")

    for i in range(7):
        ordner = archiv_basis / f"2026-04-{10 + i:02d}_080000"
        ordner.mkdir(parents=True)
        (ordner / f"modell_alt_{i}.lgbm").write_text("x")

    tmp = tmp_path / "hecke_tmp"
    tmp.mkdir()
    (tmp / "modell_neuer.lgbm").write_text("neu")
    _swap_live_mit_tmp(
        live, tmp, archiv_basis / "2026-05-20_080000",
        archiv_behalten=5,
        geschuetzte_dateien={"modell_alt_0.lgbm"},
    )

    verbleibend = sorted(p.name for p in archiv_basis.iterdir())
    assert "2026-04-10_080000" in verbleibend, (
        f"referenzierter Ordner wurde geloescht: {verbleibend}"
    )
    # Nur der referenzierte Alt-Ordner ueberlebt zusaetzlich zu den 5
    # juengsten (der Swap-Ordner ist einer davon).
    assert len(verbleibend) == 6


def test_swap_live_mit_tmp_schuetzt_aktuelles_symlink_ziel(tmp_path):
    """T-0478 (b): das `aktuell_*`-Ziel wird VOR dem Rename aufgeloest.

    Rollback-Fall: das live verlinkte Modell liegt gleichnamig auch in
    einem alten Archiv-Ordner. Der darf nicht wegfallen, sonst greift ein
    Rollback ins Leere -- und zwar unabhaengig von der DB-Schutzmenge,
    die hier bewusst leer ist.
    """
    live = tmp_path / "hecke"
    archiv_basis = tmp_path / "hecke_archiv"
    live.mkdir()
    (live / "modell_6h_q50_2026-05-01.lgbm").write_text("live")
    (live / "aktuell_6h.lgbm").symlink_to("modell_6h_q50_2026-05-01.lgbm")

    for i in range(7):
        ordner = archiv_basis / f"2026-04-{10 + i:02d}_080000"
        ordner.mkdir(parents=True)
        (ordner / "fremd.lgbm").write_text("x")
    # Der aelteste Ordner enthaelt dasselbe Modell wie der Live-Symlink.
    (archiv_basis / "2026-04-10_080000"
     / "modell_6h_q50_2026-05-01.lgbm").write_text("kopie")

    tmp = tmp_path / "hecke_tmp"
    tmp.mkdir()
    (tmp / "modell_6h_q50_2026-05-20.lgbm").write_text("neu")
    _swap_live_mit_tmp(
        live, tmp, archiv_basis / "2026-05-20_080000",
        archiv_behalten=5, geschuetzte_dateien=frozenset(),
    )

    verbleibend = sorted(p.name for p in archiv_basis.iterdir())
    assert "2026-04-10_080000" in verbleibend, (
        f"Symlink-Ziel nicht geschuetzt: {verbleibend}"
    )
...
# --- T-0564: Deploy-Swap mit Rollback ---

def test_t0564_swap_rollt_zurueck_wenn_der_zweite_rename_scheitert(tmp_path):
    """Zwei `rename` hintereinander sind kein atomarer Deploy.

    Schlug der zweite fehl, war das Live-Verzeichnis weg -- und der
    `finally`-Block des Jobs loeschte danach `<ausgabe>_tmp` mit den neuen
    Modellen gleich mit. Uebrig blieb nur das Archiv. Die Folgerunde ist
    die eigentliche Gefahr: ohne Live-Modelle nimmt der Job den
    Erstlauf-Pfad und deployt JEDEN Kandidaten ohne MAE-Vergleich -- genau
    die Luecke, die T-0399 geschlossen hat.
    """
    from unittest.mock import patch

    from bewaesserung.ml.retrain_job import _swap_live_mit_tmp

    live = tmp_path / "feuchte"
    tmp = tmp_path / "feuchte_tmp"
    archiv = tmp_path / "feuchte_archiv" / "2026-09-04_120000"
    live.mkdir()
    (live / "modell_6h_ALT.lgbm").write_text("alt")
    tmp.mkdir()
    (tmp / "modell_6h_NEU.lgbm").write_text("neu")

    echt = Path.rename

    def _zweiter_rename_scheitert(self, ziel):
        if Path(self) == tmp:
            raise OSError(18, "Invalid cross-device link")
        return echt(self, ziel)

    with patch.object(Path, "rename", _zweiter_rename_scheitert):
        with pytest.raises(OSError):
            _swap_live_mit_tmp(live, tmp, archiv)

    assert live.is_dir(), "der Altstand muss wiederhergestellt sein"
    assert (live / "modell_6h_ALT.lgbm").exists()
    assert tmp.is_dir(), "die neuen Modelle bleiben liegen"


def test_t0564_swap_ohne_fehler_tauscht_weiterhin(tmp_path):
    """Gegenprobe: der Rollback-Pfad darf den Normalfall nicht stoeren."""
    from bewaesserung.ml.retrain_job import _swap_live_mit_tmp
    ...
    assert (live / "modell_6h_NEU.lgbm").exists()
    assert (archiv / "modell_6h_ALT.lgbm").exists()
