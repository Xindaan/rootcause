...
def berechne_bilanz_liter_aus_cache(
    ...
    for e in ventile:
        if e.zeitstempel < von or e.zeitstempel > bis:
            continue
        if e.aktion == VentilAktion.OEFFNEN and e.dauer_sekunden == 0 and e.liter is None:
            continue
...
def ereignis_zu_liter(
    ...
    # 3. UNBEKANNT (Heuristik unklassifiziert) oder IGNORIERT (User hat als
    #    Phantom markiert): Bilanz ueberspringen, Aufrufer markiert indikativ.
