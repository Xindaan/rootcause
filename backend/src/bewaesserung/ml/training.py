...
STANDARD_PARAMS = {
    ...
    "seed": 42,
    # T-0518, gemessen 13.08.2026: EIN Thread ist auf diesen Daten schneller
    # als zehn, und zwar deutlich. Benchmark ueber alle 13 Cluster des echten
    # 60-Tage-Fensters (58.459 Zeilen, `docs/analyse/t0518_num_threads/`):
    #
    #     num_threads=1    128,57 s
    #     num_threads=10   401,93 s   (+213 %)
    #
    # Jeder einzelne Cluster war mit einem Thread schneller, auch der groesste
    # (waldblumenhain, 12.703 Zeilen: 20,3 s gegen 38,1 s). Der Grund steht im
    # Stack-Sample desselben Tages: der LightGBM-Master verbrachte 81 % seiner
    # Zeit in `__kmp_join_barrier`, bei 2,0-2,55 genutzten Kernen von zehn --
    # die parallelen Regionen sind kuerzer als der Fork-Join-Overhead
    # ([[fehlerpattern_threadpool_auf_mini_datensatz]]).
    #
    # Damit ist die alte Begruendung "die Produktivdatensaetze sind gross
    # genug, dass der Pool sich lohnt" (T-0518, urspruenglicher Tasktext) an
    # echten Daten widerlegt -- sie war eine Herleitung, keine Messung.
    #
    # **Wann das neu zu pruefen ist:** wenn die Datenmenge je Cluster
    # groessenordnungsmaessig waechst. Der Abstand ist beim groessten Cluster
    # am kleinsten (Faktor 1,9 statt 4,1), die Umkehr liegt also irgendwo
    # jenseits davon. Nachmessen mit
    # `docs/analyse/t0518_num_threads/benchmark.py`, nicht schaetzen.
    "num_threads": 1,
}



# T-0564: geforderte Mindestverbesserung der Abdeckung (Prozentpunkte
# als Anteil), damit das Gate nicht auf Rundungsrauschen reagiert.
MIN_ABDECKUNGS_GEWINN = 0.01


def band_scale_akzeptabel(
    abdeckung_roh: float, abdeckung_nach_scale: float, ziel: float,
) -> bool:
    """T-0564: Bringt der Skalierfaktor die Abdeckung naeher ans Ziel?

    `abdeckung_nach_scale` wurde vorher berechnet, geloggt -- und nie
    geprueft. Der Scale wurde auch geschrieben, wenn er die Abdeckung
    nachweislich VERSCHLECHTERT. In den 42 deployten `band_scale_*.json`
    traf das auf fuenf zu, u. a. `magerwiese 6h: 0,886 -> 0,367` und
    `pilea 6h: 0,988 -> 0,450`.

    Ursache ist die lineare Formel `ziel / abdeckung` -- der Kommentar an
    der Rechenstelle verspricht eine Root-Daempfung, der Code hat keine.
    Statt die Formel zu raten, wird hier das ERGEBNIS gemessen: ein Band,
    das ehrlich zu breit ist, ist besser als eines, das falsch skaliert
    wurde.

    Die Mindestverbesserung von einem Prozentpunkt ist kein Feinschliff,
    sondern noetig: ohne sie entscheidet an der Grenze das
    Fliesskomma-Rauschen. `abs(0.90 - 0.80)` ist 0,09999999999999998 und
    `abs(0.70 - 0.80)` ist 0,10000000000000009 -- ein Sprung von 0,70 auf
    0,90 galte damit als Verbesserung, obwohl er das Ziel nur auf der
    anderen Seite genauso weit verfehlt.
    """
    return (
        abs(abdeckung_nach_scale - ziel)
        < abs(abdeckung_roh - ziel) - MIN_ABDECKUNGS_GEWINN
    )
...
class TrainingsPipeline:
    ...
    def _kalibriere_band_scales(
        ...
        for h in horizonte:
            ...
            # T-0564: Akzeptanz-Gate. `abdeckung_nach_scale` wurde berechnet,
            # geloggt -- und nie geprueft. Der Scale wurde auch geschrieben,
            # wenn er die Abdeckung nachweislich VERSCHLECHTERT. In den 42
            # deployten `band_scale_*.json` traf das auf fuenf zu, u. a.
            # `magerwiese 6h: 0,886 -> 0,367` und `pilea 6h: 0,988 -> 0,450`.
            #
            # Ursache ist die lineare Formel `ziel/abdeckung` -- der
            # Kommentar darueber verspricht Root-Daempfung, der Code hat
            # keine. Statt die Formel zu raten, wird das Ergebnis gemessen:
            # verfehlt der skalierte Wert das Ziel weiter als der rohe,
            # bleibt es bei 1.0. Ein Band, das ehrlich zu breit ist, ist
            # besser als eines, das falsch skaliert wurde.
            if scale != 1.0 and not band_scale_akzeptabel(
                abdeckung_roh, abdeckung_nach_scale, ziel_abdeckung,
            ):
                if True:
                    logger.warning(
                        "band_scale.verworfen horizont=%dh scale=%.3f "
                        "abdeckung_roh=%.3f abdeckung_nach_scale=%.3f",
                        h, scale, abdeckung_roh, abdeckung_nach_scale,
                    )
                    scale = 1.0
                    halbbreite = (q_hi - q_lo) / 2
                    abdeckung_nach_scale = abdeckung_roh
    ...
    def _walk_forward_cv(
        ...
        for fold in range(self._n_folds):
            ...
            # T-0564: Early Stopping darf NICHT auf dem Testblock laufen.
            # Bis hier war `ds_valid` aus `X_test/y_test` gebaut und
            # `predict()` nahm `best_iteration` -- die berichtete CV-MAE war
            # damit das Minimum ueber die Iterationen AUF DEN TESTDATEN.
            # Und genau diese Zahl ist die Deploy-Gate-Metrik und der Wert
            # in `/api/ml/status`. Auf Daten ohne lernbares Signal
            # gegengerechnet: 2,108 gegen 2,561 bei ehrlicher Validierung,
            # also rund 18 % zu optimistisch.
            #
            # Der Validierungsblock ist jetzt das Ende des TRAININGS-Blocks.
            # Er liegt vor `train_ende` und damit vor dem Gap zum Test --
            # keine Beruehrung mit den Daten, auf denen gemessen wird.
            valid_groesse = max(30, int(train_ende * 0.15))
            inner_ende = train_ende - valid_groesse
            fruehes_stoppen = inner_ende >= 100

            if fruehes_stoppen:
                X_fit, y_fit = X.iloc[:inner_ende], y[:inner_ende]
                X_val = X.iloc[inner_ende:train_ende]
                y_val = y[inner_ende:train_ende]
                w_fit = (
                    sample_weights[:inner_ende]
                    if sample_weights is not None else None
                )
                w_val = (
                    sample_weights[inner_ende:train_ende]
                    if sample_weights is not None else None
                )
            else:
                # Zu wenig Daten fuer einen eigenen Block. Dann lieber ganz
                # ohne Early Stopping trainieren als die Rundenzahl am
                # Testblock zu waehlen -- eine feste Rundenzahl ist
                # vielleicht suboptimal, aber sie luegt nicht.
                X_fit, y_fit = X.iloc[:train_ende], y[:train_ende]
                w_fit = (
            ...
            callbacks = [lgb.log_evaluation(period=0)]  # Still
            valid_sets = []
            valid_names = []
            if fruehes_stoppen:
                ...
                valid_names = ["valid"]
                callbacks.insert(
                    0, lgb.early_stopping(stopping_rounds=20, verbose=False),
                )
            ...
            ergebnisse.append({
                "fold": fold,
                # T-0564: was tatsaechlich gefittet wurde. Bei aktivem
                # Early Stopping ist das der Block OHNE die inneren
                # Validierungszeilen.
                "train_n": len(X_fit),
                "test_n": len(X_test),
