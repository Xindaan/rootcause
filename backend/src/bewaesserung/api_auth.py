...
ROUTE_ROLLEN: dict[tuple[str, str], Rolle] = {
    ...
    ("GET", "/api/ops/request-raten"): "read",  # T-0463
...
# T-0557: control-Routen, die vom Ratelimit ausgenommen sind.
#
# Das Limit (5/min/Key) schuetzt gegen versehentliche Schleifen und gegen
# einen entwendeten Key, der Wasser AUFDREHT. Diese drei Routen koennen
# ausschliesslich SCHLIESSEN -- sie sind die Gegenrichtung, und ein 429 auf
# ihnen ist selbst der Schaden. Realfall aus dem Review 02.09.2026: fuenf
# vorangegangene control-Aufrufe (auch fehlgeschlagene, das Limit zaehlt vor
# der Ausfuehrung) liessen `POST /api/notfall-stopp` mit 429 antworten,
# waehrend ein Ventil offen war. Genau die Klick-Folge, die im Notfall
# passiert: erst mehrfach stoppen versuchen, dann den roten Knopf.
#
# Bewusst alle drei und nicht nur der Notfall-Stopp (Isomorphie-Check): die
# Fehlerklasse ist "sicherheitserhoehende Aktion teilt den Eimer mit den
# sicherheitssenkenden", und `manuell-stop` / `pre-soak-stop` schliessen
# genauso. Ein Missbrauch dieser Routen kostet Wasser, das nicht floss.
RATELIMIT_AUSNAHMEN: frozenset[tuple[str, str]] = frozenset({
    ("POST", "/api/notfall-stopp"),
    ("POST", "/api/ventil/manuell-stop"),
    ("POST", "/api/ventil/pre-soak-stop"),
})
...
async def auth_dependency(request: Request) -> None:
    ...
    if rolle_noetig == "control" and schluessel not in RATELIMIT_AUSNAHMEN:
