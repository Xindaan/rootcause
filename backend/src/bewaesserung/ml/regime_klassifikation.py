...
eines Audit-Snapshots in genau ein Regime:
    ...
    regen_unbekannt             — wetter_archiv deckt das Fenster nicht ab
                                  (Archiv-Lag) -> Regen nicht beurteilbar
...
class RegimeKontext:
    ...
    # Juengster wetter_archiv-Zeitstempel des Standorts (Archiv-Lag-Grenze).
...
async def lade_regime_kontext(
    ...
    for e in events:
        ...
        if e.zone_id == zone_id:
            ...
            # T-0470: "hat die Quell-Zone physisch beregnet?" wurde hier
            # anders beantwortet als in `sensor_backfill._cross_spray_quelle`
            # (dort `ventil_id != 'sensor_heuristik'`, hier gar kein Filter)
            # -- zweite Wahrheit fuer dieselbe Frage, Verstoss gegen
            # Heuristik 1 der Projekt-CLAUDE.md.
            #
            # Ein Heuristik-Ereignis der Quell-Zone heisst "dort gab es einen
            # Feuchtesprung ohne Erklaerung". Das ist gerade KEIN Beleg dafuer,
            # dass ihr Regner lief -- und die FREMDWASSER-Paare aus T-0469
            # tragen dieselbe `ventil_id`, koennten sich ueber wechselseitig
            # konfigurierte Zonen also endlos weitertragen. Folge bisher:
            # Fenster wurden als `ausgeschlossen_cross_spray` etikettiert,
            # obwohl nachweislich kein Regner lief, und fielen damit aus der
            # Regime-Auswertung.
            #
            # Bewusst OHNE Ausloeser-Filter, exakt wie die Referenz: ein
            # `ignoriert`- oder `gardena_web`-Lauf der Quell-Zone ist echtes
            # Wasser, der Regner lief physisch.
    ...
    if standort_id:
        ...
        # Archiv-Lag-Grenze: juengster Archiv-Eintrag GLOBAL fuer den
