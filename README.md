...
- **Bewaesserung ausfuehren** — automatisch (Opt-in pro Zone) oder manuell,
  inkl. Pre-Soak gegen hydrophobe Substrate und Cycle-and-Soak fuer Sandboden.
- **Giessfenster aus der Verdunstung** — optional statt fester Uhrzeiten: ein
  Start ist erlaubt, wenn danach genug Verdunstung zum Abtrocknen folgt (kein
  nasses Laub ueber Nacht) und waehrenddessen nicht zu viel (keine pralle
  Sonne). Beides aus der stuendlichen ET0-Vorhersage.
- **Geteilter Haupthahn** — ein zentraler Entscheider verteilt den Wasserdruck:
  exklusive Zonen (Sprinkler) laufen allein, eine Pre-Soak-Sequenz behaelt den
  Hahn ueber ihre Einsickerpause, ausser eine Zone mit heute verfallendem
  Zeitfenster (`bevorzugte_zeiten`) braucht ihn. Eine Wache meldet, wenn eine Zone zu lange auf den
  Hahn wartet.
...
- Python 3.11+ (siehe `backend/pyproject.toml`), Node.js (fuer den Frontend-Build).
...
Was sich wann geaendert hat: [CHANGELOG.md](CHANGELOG.md). Der oeffentliche
Stand entsteht als Snapshot aus einem privaten Arbeits-Repo — die Git-History
hier zeigt deshalb nur den aktuellen Stand, nicht die Entwicklung.
...
pip install -e "backend[ml]"          # [ml] = pandas/scikit-learn/lightgbm
...
# Empfohlen: FYTA_EMAIL, FYTA_PASSWORD (fuer FYTA-Pflanzensensoren; das Token
#            holt und erneuert das Backend dann selbst)
...
**Übersicht (neu)** — zusätzliche Testansicht am Ende der Tab-Leiste

- Kompakte Zonenliste mit Standortfilter und Filter „Mit Hinweisen“; rechts die ausgewählte Zone. Auf dem Handy öffnet ein Zonenklick die Details, „Zur Zonenliste“ führt zurück.
- Hinweiszahl, Kurzliste und Detailkopf verwenden dieselbe Bewertung aus dem Dashboard-Snapshot.
- Messwert, Quelle, Messalter, Zielbereich und gültige 24h-Modellprognose mit q10/q90-Spanne bleiben getrennt sichtbar.
- Fehlende oder veraltete Daten, Lead-Ausfall und Kalibrierung erhalten einen eigenen Zustand. Bei einem fehlgeschlagenen oder über zwei Minuten alten Snapshot werden Empfehlungen und Prognosen ausgesetzt; der letzte Datenstand bleibt gekennzeichnet lesbar.
- Steueraktionen („Gegossen“, „Live starten“, „Pre-Soak“, laufende Bewässerungen) nutzen die vorhandene Steuerung; der globale Notfall-Stopp bleibt erhalten.
- „Verlauf und Analyse“ lädt die bestehenden Charts, Sensor-Diagnosen, ML-Details und Wasserbilanz.

Frontend-Prüfung: `cd frontend && npm run test:overview && npm run test:segmente && npm run test:bilanz && npm run test:giessfenster && npm run build && npm run lint`.
...
**Übersicht** — der erste Tab und die Startansicht

Kartenansicht mit Triage und Detail-Drawer, Implementierung unter
`frontend/src/komponenten/zonen-karte-v4/`. Darüber das **KRITISCH-Band**:
Zonen, deren Messwert unter der kritischen Schwelle liegt, oder deren
24-h-Prognose unter die Mindestschwelle fällt, sofern die Entscheidungs-Engine
ebenfalls Bedarf sieht (eine Prognose allein löst keinen Alarm aus). Zonen mit
wahrscheinlichem Sensordefekt erscheinen dort nicht.

**Lead-Sensor ausgefallen:** hat eine Zone einen `aggregat_lead_geraet` (den
einen Sensor, der für sie maßgeblich ist) und liefert der gerade keinen
aktuellen Wert, meldet das Dashboard **nicht** „kritisch“ auf einem der
übrigen Sensoren. Die Zone gilt dann als unbekannt, und Band wie Karte nennen
den Sensor, aus dem die angezeigte Zahl tatsächlich stammt. Der Lead ist ja
gerade deshalb gesetzt, weil die anderen Sensoren nicht belastbar sind. Die
API liefert dazu `feuchte_geraet_id`, `lead_ausgefallen` und
`aggregat_lead_geraet` pro Zone.
...
1. **Glance** (Sekunden-Blick): Header mit Zonen-Name, `KANAL X ·
   ...
   **Die Karte zeigt keine Zahl, die sie nicht belegen kann:** die
   24h-ML-Prognose wird durch `24h: veraltet` bzw. `24h: Sensortausch`
   ersetzt, wenn sie auf einer Messzeile rechnet, die mehr als 3 h hinter
   der juengsten Messung der Zone liegt, oder wenn unter derselben
   `geraet_id` inzwischen ein anderes physisches Geraet steckt. Der Trend
   rechnet nur auf `aggregat_lead_geraet` und nicht ueber eine
   Kalibrier-Fenstergrenze hinweg; bei gemischten Sensoren ohne Lead
   zeigt er `–` statt einer Zahl aus zwei Skalen.
   **Gegenrichtung:** faellt der Messwert aus dem 4-h-Abruf-Fenster, steht
   statt `-` der letzte bekannte Wert gedaempft mit seinem Alter
   (`10 ⚠ · vor 4 h`). Eine Empfehlung leitet sich daraus nicht ab; das
   Entscheidungs-Fenster bleibt bei 4 h.
...
3. **Inspect** (Tief-Dive, auf-/zuklappbar): Feuchte-Verlauf-Chart
   mit ML-Prognose-Band (24h/48h/7d/30d toggle; die Feuchte-Linie ist an
   den Grenzen eines `sensor_kalibrierung`-Fensters unterbrochen, damit ein
   Sensortausch nicht als Trocknung durchgezeichnet wird), Modell-Drift-Ampel
...
- **Watchdog-Timer**: schliesst das Ventil nach der KOMMANDIERTEN Dauer + 30 s (nicht nach `max_dauer_sekunden`; der Wert deckelt die Dauer nur bei der Berechnung).
...
python -m bewaesserung.api_auth schluessel-erstellen --rolle control --label mein-iphone

# Alle Schluessel listen
python -m bewaesserung.api_auth liste

# Schluessel entfernen
python -m bewaesserung.api_auth loeschen --label mein-iphone
```
...
| `/api/ventil/manuell-start` | POST | Live-Bewaesserung einer Zone, echter Ventilbefehl (`zone_id`, `dauer_sekunden`). Wirkt auch bei `ventilsteuerung_aktiv: false`; das Flag steuert nur den Auto-Loop |
| `/api/ventil/manuell-stop` | POST | Laufende Live-Bewaesserung sofort stoppen (`zone_id`) |
| `/api/ventil/pre-soak-start` | POST | Pre-Soak-Sequenz (Vorwasser + Pause + Hauptdose) als Hintergrund-Task (`zone_id`, `haupt_min`, optional `pre_soak_min`, `pause_min`) |
| `/api/ventil/pre-soak-stop` | POST | Laufende Pre-Soak-Sequenz abbrechen und Ventil sofort stoppen (`zone_id`) |
...
- **Zonen**: Schwellen, Budget, bevorzugte Zeiten, Modus, `flaeche_m2`, `anteil_kanal` (Anteil am gemeinsamen Kanal-Wasser). Optional pro Zone:
  ...
  - `ml_dosis_opt_in` — Default `false`. Schaltet die ML-gestuetzte Dosisberechnung **pro Zone** frei. Bewusst kein globaler Schalter: ob ML oder Heuristik besser trifft, faellt je Zone gegensaetzlich aus — ein pauschales `true` verschlechtert die eine Zone, waehrend es die andere verbessert. Erst gegen den eigenen Audit-Trail pruefen, dann pro Zone freigeben.
  - `ausfall_schwelle_stunden` — ab wann eine Zone als "kein Messwert" gilt. Ohne Angabe greift der globale Default (Gardena 3 h, FYTA 12 h). Sensoren mit systematisch langsamer Cadence — etwa Bluetooth-Pflanzensensoren am Rand der Hub-Reichweite, die nur alle 2–4 Tage durchkommen — brauchen einen deutlich groesseren Wert, sonst meldet das System dauerhaft Ausfall.
  - `feuchte_null_ist_defekt` — Default `true`: ein Messwert von exakt 0,0 % gilt als Kontaktverlust des Sensors, nicht als Trockenheit. Fuer Zonen, deren Sensor 0,0 als echten Randwert liefern kann, hier auf `false` setzen. Der Guard entscheidet zusaetzlich anhand der Trajektorie, nicht am Einzelwert.
  - `giessfenster_et0` — Gießfenster aus der Verdunstung statt aus der Uhrzeit. `modus`: `aus` (nur `bevorzugte_zeiten`), `schatten` (Default: Verhalten unverändert, die Bedingung wird nur ausgewertet und geloggt, zum Kalibrieren) oder `aktiv` (`klammer` ersetzt `bevorzugte_zeiten`, die Bedingung schneidet darin zu). Ein Start ist erlaubt, wenn die ET0-Summe in `trocknung_fenster_h` Stunden **nach** dem Zyklus mindestens `trocknung_min_mm` beträgt (kein nasses Laub über Nacht) und die ET0-Summe **während** des Zyklus höchstens `sonne_max_mm` (nicht in die pralle Sonne). Ist eine der beiden Schwellen nicht gesetzt, wird diese Seite nicht geprüft. `zyklus_min` ist Pflicht und muss der realen Zyklusdauer entsprechen. Der Kritisch-Bypass gilt unverändert.
  - `wirkungs_alarm_aktiv` — Default `true`. Schaltet pro Zone die Prüfung „Bewässerung ohne Wirkung“ ab, für Sensoren, die an ihrer Position bauartbedingt keinen Anstieg zeigen können (z. B. bewusst außerhalb der Tropfzone als Kontrolle). Preis: für die Zone entfällt die automatische Kontrolle, ob Wasser ankommt. Vorher prüfen, ob eine andere Zone am selben Ventil das mit abdeckt.
  - `eingefroren_alarm_aktiv` — Default `true`. Gegenstück für die Prüfung „Sensor eingefroren“, für Sensoren, deren Konstanz ein dokumentierter Zustand ist und kein Defekt (z. B. trockener Sand unter der Auflösung der Skala, der dauerhaft 0,0 meldet).
  - **Weitere Zonen-Felder** (alle unter `zonen[]`, alle optional; ohne Eintrag gilt der genannte Default):
    - Budget und Takt:
      - `tages_budget_sekunden` — Default `3600`. Harte Obergrenze der Ventil-Sekunden pro Tag im Automatik-Betrieb; ist sie erreicht, wird nicht mehr gegossen (Runaway-Schutz). Die frühere Angabe `tages_budget_liter` wird beim Laden abgelehnt, weil Liter nicht in Sekunden umrechenbar sind.
      - `tages_budget_kritisch_faktor` — Default `1.0`. Bei kritischer Trockenheit darf bis Faktor × Tagesbudget gegossen werden. Gedeckelt, damit ein hängender Niedrig-Sensor nicht endlos gießt.
      - `tages_advisory_anteil` — Default `1.0`. Ab welchem Anteil des Tagesbudgets der manuelle Gießpfad warnt. Bewusst getrennt vom Budget, das als Notbremse großzügig über dem Bedarf steht.
      - `min_pause_minuten` — Default `120`. Mindestabstand zwischen zwei Läufen der Zone, gemessen ab Öffnen. Bei kritischer Trockenheit darf die Pause übersprungen werden (siehe nächster Punkt).
      - `pause_bypass_karenz_stunden` — Default `1.0`. So lange nach Laufende bleibt dieser Kritisch-Bypass der Mindestpause gesperrt; entspricht dem Sensor-Nachlauf von etwa einer Stunde.
    - Kanal mit mehreren Zonen:
      - `ventil_name` — Name des Ventils in der Gardena-App, falls er vom Zonennamen abweicht. Die Kanal-Zuordnung vergleicht ohne Groß-/Kleinschreibung; `ventil_name` hat Vorrang vor `name`.
      - `kanal_trigger_ausschluss` — Default `false`. Die Zone löst den Kanal-Start nicht aus (bleibt in Durchschnitt und Log). Für einen unzuverlässigen Sensor an einem geteilten Ventil; der verlässliche Partner führt.
      - `kanal_max_stop_ausschluss` — Default `false`. Gegenstück: die Zone beendet den Kanal-Lauf nicht über ihren Maximalwert. Für einen Sensor direkt in der benetzten Zone am Tropfer, der voll meldet, bevor die Zone versorgt ist.
      - `kanal_max_stop_notbremse_pp` — Default `95`. Harte Obergrenze für eine Zone mit `kanal_max_stop_ausschluss`, damit der Kanal nicht ganz ohne Bremse läuft. Unter 100, weil am Skalenende „sehr nass“ nicht mehr von „übersättigt“ zu unterscheiden ist.
      - `cross_spray_quell_zonen` — Default `[]`. Zonen, deren Regner den Sensor dieser Zone mit benetzen. Ein Feuchtesprung ohne eigenen Lauf, während eine Quell-Zone lief, gilt dann nicht als eigene Bewässerung.
    - Wirkung und Dosis:
      - `wirkung_max_pp` / `wirkungsrate_initial` — Default aus. Plateau-Modell der Gießwirkung: Gesamtanstieg nach `d` Minuten = `wirkung_max_pp * (1 - exp(-d / tau))` mit `tau = wirkung_max_pp / wirkungsrate_initial` (Anfangsrate in pp/min). Verhindert, dass lange Dosen linear hochgerechnet werden. Beide setzen; hat Vorrang vor `wirkungsrate_dauer_alpha`.
      - `wirkungsrate_dauer_alpha` — Default `0.0`. Log-Korrektur der Wirkungsrate bei langen Läufen: `rate(d) = rate0 * (1 + alpha * log(d / 30))`. Negative Werte senken die Rate langer Läufe (Tiefensickerung); Untergrenze 0,05 pp/min.
      - `regen_faktor_pp_pro_mm` — Default aus (globaler Wert 4,0). Feuchtezuwachs pro mm Regen in dieser Zone (Blattdach, Kronentraufe). Nur mit längerer Messreihe setzen; Einzelereignisse streuen stark.
      - `regen_erreicht_ballen` — Default `true`. Ob Regen den Wurzelballen erreicht. Für überdachte Töpfe `false`, sonst leitet die Kalibrierung die Feldkapazität aus einem Regen ab, der nie ankam.
      - `k_basis_pro_h` — Default aus. Fester Wert für die Trocknungskonstante des Physik-Moduls in 1/h (typisch 0,005 bis 0,05). Ohne Eintrag gilt der gefittete Wert, ohne Fit ein Standardwert.
    - Proaktiver Trigger:
      - `proaktiv_tage_vor_optimum_min` — Default aus. Die Engine schlägt „präventiv“ vor, sobald die Physik-Reserve bis `optimum_feuchte_min` höchstens so viele Tage beträgt, statt erst am Akut-Rand. Für die Strategien `selten_gross` und `korridor`.
      - `proaktiv_min_sensor` — Default `false`. Bei mehreren Sensoren stützt sich der proaktive Trigger auf den trockensten statt auf den Median. Betrifft nur diesen Trigger.
    - Sensor-Heuristik (Bewässerungen aus Feuchtesprüngen erkennen) und Wirkungsprüfung:
      - `heuristik_min_delta_pp` — Default aus (global 5 pp). Kleinerer Mindestsprung pro Messintervall, für Sensoren, die je Intervall nur 2 bis 3 pp steigen (Topf mit Tropfer).
      - `heuristik_rollup_fenster_min` / `heuristik_rollup_schwelle_pp` — Default aus. Zusätzlich zur Einzelsprung-Prüfung: steigt die Feuchte innerhalb des Fensters (Minuten) um mindestens die Schwelle (pp), gilt das als Bewässerung. Für langsame, über Stunden verteilte Anstiege. Wirkt nur, wenn beide gesetzt sind.
      - `detektor_fenster_ende_min` — Default aus (global 90 min). Wie lange die Prüfung „Bewässerung ohne Wirkung“ nach dem Schließen des Ventils auf einen Anstieg wartet. Für tiefe Wurzelballen und große Regnerflächen länger (etwa 180).
    - ML und Kalibrierung:
      - `cluster_id` — Default = `zone_id`. Zonen mit gleicher `cluster_id` teilen ein ML-Feuchtemodell; nur für bewusst identisch platzierte Sensoren.
      - `ist_indoor` / `ist_topf` — Default `false`. Merkmale für das ML-Feature-Engineering (Innenpflanze ohne Regen und Wind; Topf statt Freiland).
      - `calibration_pair_quellen` — Default `[]`. Sensorquellen (z. B. `["fyta"]`), die direkt neben dem Hauptsensor stecken; sie dienen dem Skalen-Abgleich zwischen Sensortypen.
      - `auto_loop_scharf_seit` — Zeitpunkt mit Uhrzeit, ab dem die Zone im Auto-Loop echtes Wasser bekam. Automatik-Läufe davor waren Shadow ohne Wasser und bleiben aus dem Response-Training draußen; ohne Eintrag gilt jeder Automatik-Lauf dieser Zone als Shadow.
    - Erinnerungen:
      - `benachrichtigung` — Default aus. Block für Gieß-Erinnerungen von Zonen im Modus `monitoring`: `typ` (Default `imessage`), `empfaenger` und `cooldown_stunden` (Default `24`, Mindestabstand zwischen zwei Erinnerungen derselben Zone).
...
- **ml.bewaesserungs_response**: Pro-Zone-Modell fuer empfohlene Giess-Dauer. `aktiv` (Shadow-Logging), `wirksam` (scharf statt Heuristik), `min_events`, `min_events_pump_zone`; Auto-Retrain mit eigenem Gate ueber `retrain_intervall_tage`, `retrain_event_schwelle`, `gate_mae_faktor`. Drift-Eval-Endpoint `/api/ml/dauer-drift`. Achtung: der YAML-Pfad ist `ml:` -> `bewaesserungs_response:`; ein Top-Level-Block `ml_bewaesserungs_response:` oder `bewaesserungs_response_retrain:` wird nicht gelesen (so stand es bis 18.09.2026 hier).
- **watchdog.hahn_wache**: Wartezeit-Wache am geteilten Haupthahn. `aktiv` (Default `false`), `schwelle_stunden` (Default 6), `zonen` (explizite Liste; leer = Wache aus). Wartet eine der Zonen länger als die Schwelle auf den Hahn, kommt eine Meldung. Ein Netz, kein Regler.
- **dosis_test**: randomisierter Dosis-Versuch an einem Ventil, um zu messen, ab welcher Dauer ein Sensor überhaupt reagiert. `aktiv` (Default `false`, fehlender Block = exakter No-op), `ventil_geraet_id`, `ventil_kanal`, `stufen_gesamt_min` (z. B. `[45, 60, 75]`), `wiederholungen`, `seed` (reproduzierbare Reihenfolge), `bis` (Enddatum `YYYY-MM-DD`). **Fährt echtes Wasser**, wenn aktiv.
- **feature_bau_subprozess** (unter `ml.retrain` und `ml_bewaesserungs_response`): baut die Trainings-Features in einem eigenen Prozess, damit der Event-Loop (Ventil-Callbacks, Dashboard) währenddessen nicht blockiert. Fällt der Subprozess aus, weicht der Job selbst auf den alten Weg aus. Modell-Default `false`, weil der Subprozess die Standard-Konfig und damit die Produktiv-Datenbank lädt und in Tests sonst die echte DB lesen würde; für den Betrieb in der Konfig auf `true` setzen.
...
- **Keine FYTA-Daten**: `FYTA_EMAIL`/`FYTA_PASSWORD` in `.env` gesetzt? Damit erneuert das Backend das Token selbst. Nur beim Legacy-Pfad (`FYTA_ACCESS_TOKEN`) muss man es von Hand nachtragen.
...
- **WebSocket-Event fehlt**: Heartbeat-Zeile `ws_events=...` im Log zeigt die Zaehler pro Ereignistyp; bleibt der Zaehler stehen, kommt die Verbindung nicht durch (Firewall/VPN) oder das Geraet meldet nicht mehr.
...
frontend/src/
  ...
  komponenten/
    zonen-karte-v4/        — Karten des Tabs "Übersicht" (Glance/Action/Inspect)
    uebersicht-neu/        — Testansicht "Übersicht (neu)" + Bewertungsmodell
...
| **Herkunft an der Prognose** | Jede Live-Prognose traegt `feature_zeitstempel` (auf welcher Messzeile gerechnet wurde) und `inferenz_geraet_id`. Die API liefert dazu `gueltig` + `ungueltig_grund` (`veraltet` / `geraetewechsel` / `herkunft_unbekannt`) und `feature_rueckstand_h`. Grund: ein Ausschlussfenster kann den Feature-Frame einer Zone einfrieren; die Inferenz rechnet dann korrekt, aber dauerhaft auf derselben alten Zeile. Der Geraetewechsel wird an der `sensor_id` (MAC) im Geraetestatus belegt, nicht geraten. |
...
**Stack**: Python >= 3.11 (laeuft auf 3.13) + asyncio, FastAPI, aiosqlite, LightGBM, httpx, structlog | React 19, Recharts, Vite, TypeScript
