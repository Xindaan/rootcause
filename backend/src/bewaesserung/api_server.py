...
async def zonen_uebersicht():
    ...
    for zone in _konfig.zonen:
        ...
        letzter_wert = await _speicher.letzte_messung_aggregiert(zone.zone_id)
        if not letzter_wert:
            # Fallback auf Verarbeiter-Cache (nur wenn DB leer)
            letzter_wert = _verarbeiter.hole_letzten_wert(zone.zone_id)
...
async def messwerte(
    zone_id: str,
    stunden: int = Query(24, description="Zeitraum in Stunden (ignoriert wenn von/bis gesetzt)"),
    ...
    assert _speicher
    if von or bis:
        von_dt = datetime.fromisoformat(von) if von else None
        bis_dt = datetime.fromisoformat(bis) if bis else None
...
async def ereignisse(
    zone_id: str,
    stunden: int = Query(24, description="Zeitraum in Stunden"),
):
    ...
    if _wetter_manager is not None:
        for standort in (_konfig.standorte or []):
            ...
            wetter_pro_standort[sid] = {
                "standort_id": sid,
                "regen_summe_mm": round(
                    sum(s.niederschlag_mm for s in tag_stunden), 1,
                ),
                "max_temp_c": (
                ...
                "et0_summe_mm": round(
                    sum(s.et0_mm for s in tag_stunden), 2,
                ),
            }
    ...
    for zone in _konfig.zonen:
        try:
            ...
            eintraege.append({
                ...
                "dauer_min": None,
                "bevorzugte_zeiten": [
                    f"{z.von}-{z.bis}" for z in zone.bevorzugte_zeiten
                ],
        ...
        if empf.dauer_s_empfehlung:
            dauer_min = int(round(empf.dauer_s_empfehlung / 60))
...
async def dashboard_snapshot(
    ...
    _cache_key = (zone_id, fenster, ml_details)
    ...
    letzte_pro_zone = await _speicher.letzte_messung_aggregiert_bulk(zone_ids)
    ...
    def _letzter_wert_fuer(zid: str):
        ...
        return w if w else _verarbeiter.hole_letzten_wert(zid)
            ...
            for key, v in ergebnisse_roh.items():
                ...
                if v.top_features is not None:
                    eintrag["top_features"] = [
                        {
                            "name": f.name, "wert": f.wert,
                            "beitrag": f.beitrag, "skala": f.skala,
                        }
    ...
    _SNAPSHOT_CACHE[_cache_key] = (time.monotonic(), _content)
    return JSONResponse(
        content=_content,
...
async def ops_betriebsstatus():
    ...
    if _ml_service is not None and _ml_service.ist_verfuegbar:
        ...
        ml_block = {
            "ist_geladen": True,
            "trainiert_am": status.trainiert_am.isoformat()
    ...
    if _konfig is not None:
        ...
        if taeglich.is_dir():
            snapshots = sorted(
                (p for p in taeglich.glob("*.db") if p.is_file()),
...
async def ventil_manuell_start(daten: dict):
    ...
    # T-0151: Pre-flight Hahn-Cluster-Pruefung. Wenn das Cluster-Budget
    # ausgeschoepft ist, klare Fehlermeldung statt generischer "verweigert".
    lock = sicherung.pruefe_hahn_cluster_budget(zone.ventil_kanal)
    if not lock.erlaubt:
        return {
            "ok": False,
            "fehler": "HAHN_BELEGT",
...
async def ventil_pre_soak_start(daten: dict):
    ...
    if pre_soak_sicherung is not None:
        lock = pre_soak_sicherung.pruefe_hahn_cluster_budget(zone.ventil_kanal)
        if not lock.erlaubt:
            return {
                "ok": False,
                "fehler": "HAHN_BELEGT",
...
async def ventil_ereignisse(
    ...
    assert _speicher
    jetzt = datetime.now()
    von_dt = datetime.fromisoformat(von.replace("Z", "+00:00")).astimezone().replace(tzinfo=None) if von else jetzt - timedelta(hours=24)
    bis_dt = datetime.fromisoformat(bis.replace("Z", "+00:00")).astimezone().replace(tzinfo=None) if bis else jetzt
    ...
    for zid in ziel_ids:
        ...
        if zid != ereignis_id:
            erfolg = await _speicher.aktualisiere_ventil_ereignis(
                zid, ausloser=ausloser, liter=liter,
            )
        else:
            erfolg = await _speicher.aktualisiere_ventil_ereignis(
...
async def ml_status():
    ...
    if _ml_retrain_job is not None:
        basis["retrain"] = {
            ...
            "letzter_erfolg": (
                _ml_retrain_job.letzter_erfolg.isoformat()
    ...
    if _ml_response_retrain_job is not None:
        basis["response_retrain"] = {
            ...
            "letzter_erfolg": (
                _ml_response_retrain_job.letzter_erfolg.isoformat()
    ...
    if _kalibrations_job is not None:
        basis["kalibrierung"] = {
            ...
            "letzter_erfolg": (
                _kalibrations_job.letzter_erfolg.isoformat()
    ...
    if _skalen_mapping_fit_job is not None:
        basis["skalen_mapping"] = {
            ...
            "letzter_erfolg": (
                _skalen_mapping_fit_job.letzter_erfolg.isoformat()
    ...
    if _k_basis_fit_job is not None:
        basis["physik_diagnose"] = {
            ...
            "letzter_erfolg": (
                _k_basis_fit_job.letzter_erfolg.isoformat()
    ...
    if _wirkung_fit_job is not None:
        basis["wirkung_fit"] = {
            ...
            "letzter_erfolg": (
                _wirkung_fit_job.letzter_erfolg.isoformat()
...
async def ml_vorhersage(
    ...
    # T-0179c: Aggregat ueber alle Sensoren der Zone (Median).
    letzter_wert = await _speicher.letzte_messung_aggregiert(zone_id)
    if not letzter_wert:
        letzter_wert = _verarbeiter.hole_letzten_wert(zone_id)
