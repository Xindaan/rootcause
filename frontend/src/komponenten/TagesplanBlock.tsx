...
import { giessfensterHinweis } from './giessfenster_hinweis'
            ...
            {plan.wetter_pro_standort.map(w => (
              <span key={w.standort_id} className="tp-wetter-eintrag">
                ...
                {w.min_temp_c !== null && w.max_temp_c !== null
                  ...
                  : '— °C'}
                {/* T-0575: fehlende Wetterdaten sind eine eigene Aussage.
                    Vorher lieferte eine gescheiterte Abfrage 0,0 mm, der
                    Block blieb einfach leer -- die Kachel sah aus wie eine
                    Trockenprognose. */}
                {w.regen_summe_mm === null ? (
                  <span
                    className="tp-wetter-fehlt"
                    title="Fuer diesen Tag liegen keine Vorhersage-Stunden vor (Wetterabfrage gescheitert). Die Zahlen fehlen, sie sind nicht null."
                  > · keine Wetterdaten</span>
          ...
          <ul className="tp-liste tp-liste-bedarf">
            {bedarf_blockiert.map(e => (
              <li key={e.zone_id} className="tp-eintrag tp-eintrag-bedarf">
                ...
                {giessfensterHinweis(e.giessfenster, new Date()) && (
                  <span className="tp-fenster">
                    {giessfensterHinweis(e.giessfenster, new Date())}
                  </span>
                )}
              </li>
