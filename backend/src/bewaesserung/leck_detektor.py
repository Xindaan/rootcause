...
# T-0544: die Tabelle wohnt jetzt in `modelle.py` -- sie hat seit der
# Plateau-Erkennung einen zweiten Leser. Hier nur noch re-exportiert, damit
# bestehende Importe (und Tests) unveraendert funktionieren.
from bewaesserung.modelle import (  # noqa: E402
    AUFLOESUNG_PP,
    AUFLOESUNG_PP_DEFAULT,
)
...
class KanalRolle:
    ...
    #: T-0438: die GIESS-Schwelle der Zone. Das Schliess-Kriterium hing
    #: frueher allein an `feuchte_kritisch` -- zwischen Giess-Schwelle und
    #: Kritisch behauptete die Warnung weiter "Lead meldet satt", obwohl der
    #: Kanal laengst ausloesen wuerde (Realfall 27.07.: Lead 55 bei Schwelle
    #: 61, Warnung stand trotzdem). Die effektive Schwelle liegt wegen des
    #: Hitzeaufschlags >= diesem Wert, ein Lead darunter giesst also sicher.
...
class LeckDetektor:
    ...
    def __init__(
        ...
        wirkungs_alarm_aus: frozenset[str] | set[str] | None = None,
        eingefroren_alarm_aus: frozenset[str] | set[str] | None = None,
    ) -> None:
        ...
        `wirkungs_alarm_aus` (T-0540): Zonen, fuer die die Pruefung
        "Bewaesserung ohne Wirkung" NICHT laeuft. Gedacht fuer Sensoren, die
        an ihrer Position bauartbedingt keine Trigger-artige Sofortreaktion
        zeigen koennen (bambuswald/aaaa0002 sitzt bewusst ausserhalb der
        Tropfer-Zwiebel) -- dort ist der Alarm strukturell garantiert und
        damit nicht diagnostisch. Die uebrigen Pruefungen der Zone laufen
        weiter. Kommt aus ZonenKonfig `wirkungs_alarm_aktiv`.
        ...
        # T-0540: Zonen ohne Wirkungs-Alarm. Leer -> alle Zonen werden
        # geprueft (Bestandsverhalten).
        self._wirkungs_alarm_aus: frozenset[str] = frozenset(
            wirkungs_alarm_aus or ()
        )
        # T-0574: Zonen ohne "Sensor eingefroren"-Pruefung. Getrennte Menge,
        # nicht dieselbe wie oben: bambuswald hat den Wirkungs-Alarm ab, soll
        # aber weiterhin auf eingefrorene Sensoren geprueft werden.
        self._eingefroren_alarm_aus: frozenset[str] = frozenset(
            eingefroren_alarm_aus or ()
        )
    ...
    def _mindest_erwartung_pp(
        self, zone_id: str, geraet_id: str | None = None,
    ) -> float:
        ...
        **T-0545: die `geraet_id` ist die eigentliche Quelle.** Bis hierher
        las diese Methode nur `zone_quellen` -- ein Konstruktor-Parameter, den
        **keine einzige Stelle im Repo je befuellt hat**, auch main.py nicht.
        Der Lookup lief also seit T-0428 (Juli) immer ins Leere und lieferte
        fuer JEDE Zone den Gardena-Default, obwohl elf der vierzehn Zonen an
        FYTA-Sensoren haengen. Wirksam war damit 10 pp statt der passenden
        2 pp: der Detektor hat fuer diese Zonen praktisch jede Auswertung als
        "unter Sensor-Aufloesung" verworfen.

        Beide anderen Aufrufer von `aufloesungs_min_pp` in diesem Modul
        (Zeilen um den Eingefroren-Check) machten es von Anfang an richtig und
        leiteten die Quelle aus der `geraet_id` ab -- nur diese eine Naht war
        nie verbunden ([[fehlerpattern_detektor_ohne_konsument]]).

        Reihenfolge: Geraet vor Zonen-Mapping vor konservativem Default.
        `zone_quellen` bleibt als Ueberschreibung erhalten, damit ein Aufrufer
        die Ableitung notfalls uebersteuern kann.
        """
        if geraet_id:
            return aufloesungs_min_pp(quelle_aus_geraet_id(geraet_id))
    ...
    async def _pruefe_bewaesserung_ohne_wirkung(
        ...
        # T-0540: Zone hat diese eine Pruefung abgewaehlt. Offene Warnung
        # schliessen, sonst haengt sie fuer immer in DB und UI -- ein
        # abgeschalteter Detektor darf keinen Zustand hinterlassen, den
        # niemand mehr aufloest (`fehlerpattern_detektor_ohne_konsument`).
        if zone_id in self._wirkungs_alarm_aus:
        ...
        vor, nach, geraet_id = _wirkungs_paar_gleiches_geraet(
        ...
        mindest_pp = self._mindest_erwartung_pp(zone_id, geraet_id)
    ...
    async def _re_check_haengende_warnung(
        ...
        for e in reversed(ereignisse):
            ...
            if e.dauer_sekunden < BEW_MIN_DAUER_SEKUNDEN:
                continue
            # T-0455: Set statt Literal -- Cloud-Zeitplan-Laeufe sind echte
            # Bewaesserungen und muessen die Wirkungs-Warnung weiter
            # rechtfertigen bzw. re-checken koennen.
            if e.ausloser not in ECHTES_KANAL_WASSER:
                continue
            alter_min = (jetzt - e.zeitstempel).total_seconds() / 60
        ...
        if kandidat is None:
            ...
            rechtfertigend = any(
                ...
                # T-0455: ECHTES_KANAL_WASSER (automatik/manuell/zeitplan)
                # plus UNBEKANNT. FREMDWASSER (T-0453) gehoert bewusst NICHT
                # dazu: fremdes Wasser rechtfertigt keine Warnung ueber die
                # ausbleibende Wirkung EIGENER Bewaesserung -- ein auf
                # `fremdwasser` geflipptes Event schliesst die Warnung damit
                # genauso wie ein auf `ignoriert` geflipptes.
                and e.ausloser in (ECHTES_KANAL_WASSER | {Ausloser.UNBEKANNT})
                for e in ereignisse
            )
        ...
        if erwartet < self._mindest_erwartung_pp(zone_id, geraet_id):
    ...
    async def _pruefe_sensor_eingefroren(
        ...
        # T-0574: Zone hat diese eine Pruefung abgewaehlt, weil die Konstanz
        # ein dokumentierter Zustand ist (magerwiese liegt unter der
        # Aufloesung ihrer Skala und meldet dort konstant 0,0). Offene
        # Warnung schliessen -- ein abgeschalteter Detektor darf keinen
        # Zustand hinterlassen, den niemand mehr aufloest
        # (`fehlerpattern_detektor_ohne_konsument`).
        if zone_id in self._eingefroren_alarm_aus:
            await self._speicher.schliesse_sensor_warnung(
        ...
        #
        # KORREKTUR 08.08.2026 (T-0526): hier stand "`SensorWarnung` hat kein
        # geraet_id-Feld -> betroffene Geraete stehen in `details` (kein
        # Schema-Umbau)". Das Feld gibt es seit T-0526. Die Erkennung war
        # schon immer geraetescharf (`pro_geraet` unten) und bleibt richtig;
        # nur die WARNUNG fasst weiterhin alle betroffenen Geraete in einen
        # zonenweiten Eintrag zusammen. Das ist jetzt eine Wahl, keine
        # Beschraenkung mehr -- eine Warnung je Geraet waere moeglich
        # (Folge-Task in TASK.md). Bis dahin gilt: der Detektor ist scharf,
        # die Meldung ist grob.
        #
        # NICHT gefangen wird ein Stufensprung
        ...
        for geraet_id, werte in sorted(pro_geraet.items()):
            ...
            geprueft += 1
            beurteilte_geraete.append(geraet_id)
        ...
        # T-0529: eine Warnung JE GERAET statt einer je Zone.
        #
        # Erkannt wurde schon immer geraetescharf; zusammengefasst wurde nur,
        # weil `SensorWarnung` bis T-0526 kein `geraet_id` hatte. Der Preis
        # war der Lebenszyklus: taut EIN Sensor auf, verschwand die Warnung,
        # obwohl ein zweiter noch eingefroren sein konnte -- dieselbe Klasse
        # wie [[fehlerpattern_dedup_pro_zone_multisensor]], wo ein
        # zonenweiter Blick tote Sensoren versteckt.
        #
        # Geschlossen wird nur fuer Geraete, die WIRKLICH beurteilt wurden.
        # Ein Sensor mit zu duenner Datenlage steht oben im `continue` und
        # darf hier nicht als "wieder gesund" gelten -- das waere die
        # Datenluecke, die eine echte Warnung versteckt (Verhalten wie vor
        # T-0391, nur eine Ebene tiefer).
        eingefroren_ids = {g for g, _, _ in eingefroren}
        for geraet_id in beurteilte_geraete:
            if geraet_id not in eingefroren_ids:
                await self._speicher.schliesse_sensor_warnung(
                    zone_id, SensorWarnungTyp.SENSOR_EINGEFROREN, jetzt,
                    geraet_id=geraet_id,
                )
        ...
        if not eingefroren:
            # Altbestand aus der Zeit vor T-0529: die zonenweite Warnung
            # (geraet_id = "") kennt kein Geraet und wuerde von der Schleife
            # oben nie erreicht.
            await self._speicher.schliesse_sensor_warnung(
                zone_id, SensorWarnungTyp.SENSOR_EINGEFROREN, jetzt,
                geraet_id="",
        ...
        def _details(geraet_id: str, spanne: float, anzahl: int) -> str:
            schwelle = (aufloesungs_min_pp(quelle_aus_geraet_id(geraet_id))
                        / AUFLOESUNG_SICHERHEITSFAKTOR)
            return (
                f"{geraet_id}: {anzahl} Messungen in "
                f"{EINGEFROREN_FENSTER_STUNDEN}h, Spanne {spanne:.2f}% "
                f"(Schwelle {schwelle:.1f}%)"
            )

        details = "; ".join(
            _details(geraet_id, spanne, anzahl)
            for geraet_id, spanne, anzahl in eingefroren
        )
        neu = False
        for geraet_id, spanne, anzahl in eingefroren:
            if await self._speicher.oeffne_sensor_warnung(SensorWarnung(
                zeitstempel=jetzt,
                zone_id=zone_id,
                geraet_id=geraet_id,
                typ=SensorWarnungTyp.SENSOR_EINGEFROREN,
                details=_details(geraet_id, spanne, anzahl),
            )):
                neu = True
        if neu:
            logger.warning(
    ...
    async def _pruefe_lead_divergenz(
        ...
        **Nicht** ein Alarm auf blosse Divergenz. Zwischen `bambuswald` und
        ...
        for andere_id, andere in self._kanal_topologie.items():
            ...
            if wert is not None:
                # T-0438: gegen die GIESS-Schwelle pruefen, nicht gegen
                # kritisch. Ein Lead unter seiner Giess-Schwelle loest den
                # Kanal aus -- dann ist der Ausschluss folgenlos und die
                # Warnung gegenstandslos, lange bevor der Lead "kritisch" ist.
                grenze = (
                    andere.feuchte_schwelle_min
                    if andere.feuchte_schwelle_min is not None
                    else andere.feuchte_kritisch
                )
                leads.append((andere_id, wert, grenze))
        if not leads:
            return
        ...
        lead_text = ", ".join(
            f"{lid} {wert:.0f}% (giesst ab {krit:.0f}%)" for lid, wert, krit in leads
        )
        # T-0438 (b): die Kausal-Aussage gegen den ECHTEN Blocker pruefen,
        # statt sie zu behaupten. "Kanal N giesst deshalb nicht" stimmte am
        # 27.07. nicht mehr: geblockt hat REGEN_ERWARTET, nicht der
        # Ausschluss. Eine Warnung, die die falsche Ursache nennt, schickt
        # den Leser an die falsche Stelle.
        ursache = await self._kanal_ursache_text(rolle, jetzt)
        details = (
            f"{zone_id} {eigen:.0f}% unter kritisch "
            f"{rolle.feuchte_kritisch:.0f}%, aber vom Kanal-Trigger "
            f"ausgeschlossen; Lead meldet satt: {lead_text}.{ursache}"
        )
        neu = await self._speicher.oeffne_sensor_warnung(SensorWarnung(
            zeitstempel=jetzt,
            zone_id=zone_id,
        ...
        if not neu:
            # T-0438 (a): `oeffne_sensor_warnung` liefert bei einer bereits
            # offenen Warnung False und laesst `details` unangetastet. Die
            # Karte zeigte damit 10 h alte Zahlen (Realfall 27.07.: Warnung
            # von 05:25, angezeigt um 15:27, inzwischen 30 -> 20 bzw.
            # 70 -> 55). Der Auffrisch-Helfer existierte seit T-0397 und
            # wurde hier nur nie gerufen -- Detektor ohne Konsument.
            await self._speicher.aktualisiere_offene_warn_details(
                zone_id, SensorWarnungTyp.LEAD_DIVERGENZ, details,
            )
        if neu:
            logger.warning(
    ...
    async def _kanal_ursache_text(self, rolle, jetzt: datetime) -> str:
        """T-0438: benennt den ECHTEN Grund, warum der Kanal nicht giesst.

        Liest die juengste KANAL-Entscheidung dieses Kanals. Nur wenn die
        wegen der Feuchtelage nicht giesst (`FEUCHTE_OK` /
        `STRATEGIE_KEIN_BEDARF`), ist der Ausschluss ueberhaupt die
        wirksame Ursache -- dann darf die Warnung das sagen. Blockt etwas
        anderes (Regen, Budget, Pause, Zeitfenster), wird DAS genannt.
        Laesst sich nichts feststellen, schweigt die Warnung zur Ursache,
        statt zu raten.
        """
        try:
            letzte = await self._speicher.hole_entscheidungen(limit=60)
        except Exception:
            return ""
        passend = [
            e for e in letzte
            if getattr(e, "scope", None) == EntscheidungsScope.KANAL
            and str(getattr(e, "scope_ref", "")) == str(rolle.kanal)
        ]
        if not passend:
            return ""
        e = max(passend, key=lambda x: x.zeitstempel)
        if e.soll_bewaessern:
            return f" Kanal {rolle.kanal} giesst gerade."
        if e.blocker_typ in (
            BlockerTyp.FEUCHTE_OK, BlockerTyp.STRATEGIE_KEIN_BEDARF, None,
        ):
            return f" Kanal {rolle.kanal} giesst deshalb nicht."
        return (
            f" Kanal {rolle.kanal} ist aktuell ohnehin geblockt "
            f"({e.blocker_typ.value})."
        )
...
def _wirkungs_paar_gleiches_geraet(
    ...
    T-0545: gibt die `geraet_id` mit zurueck. Wer das Delta zweier Werte
    bewertet, braucht die Aufloesung GENAU DIESES Sensors -- eine Zone kann
    gemischte Bauarten haben, und welche gewonnen hat, weiss nur diese
    Funktion.
    """
    ...
    for m in sorted(nach_messungen, key=lambda x: x.zeitstempel, reverse=True):
        if m.geraet_id in vor_pro and m.geraet_id in nach_pro:
            return vor_pro[m.geraet_id], nach_pro[m.geraet_id], m.geraet_id
