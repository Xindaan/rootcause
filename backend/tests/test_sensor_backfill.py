...
def test_f13_regen_check_nutzt_zone_standort_statt_default(speicher):
    """F13: Eine Outdoor-Zone mit wetter_standort=beispielstadt muss den Regen-Check
    gegen BERLIN fahren, nicht hartkodiert standort_a. Regen NUR im Beispielstadt-
    ...
    job = SensorBackfillJob(
        ...
        standort_pro_zone={"bambuswald": "beispielstadt"},
    )
    neu = _run(job.aktualisiere_wenn_faellig(jetzt=basis + timedelta(hours=2)))
    assert neu == 0
...
def test_aktive_bewaesserung_auf_kanal_trennt_dswc_geraet(speicher):
    ...
    gestartet = datetime(2026, 4, 19, 10, 0)
    ...
    # T-0447: `jetzt` explizit -- seit der Frische-Schranke ist der Check
    # zeitabhaengig, und gegen die echte Uhr waere die April-Zeile immer
    # veraltet. Hier geht es um die DSWC-Trennung, nicht um Frische.
    jetzt = gestartet + timedelta(minutes=5)
    assert _run(job._aktive_bewaesserung_auf_kanal("zone-a", jetzt)) is True
    assert _run(job._aktive_bewaesserung_auf_kanal("zone-b", jetzt)) is False


def test_t0447_veralteter_live_state_blockt_heuristik_nicht(speicher):
    """T-0447: die Frische-Schranke gab es bisher nur im Entscheidungspfad.
    Eine haengengebliebene `live_lauf_state`-Zeile (Realfall 02.07.: 12 h
    Geister-Lauf) haette die Heuristik dieses Kanals dauerhaft stumm
    geschaltet -- jeder Sprung waere als "Backend giesst gerade" verworfen
    worden, obwohl seit Stunden nichts lief.
    """
    gestartet = datetime(2026, 4, 19, 10, 0)
    _run(speicher.setze_live_lauf_state(
        kanal=1, valve_id="valve-a", geraet_id="dswc-1",
        zone_ids=["zone-a"], dauer_sekunden=600,
        ausloser=Ausloser.MANUELL.value, gestartet_am=gestartet,
    ))
    job = SensorBackfillJob(
        speicher, zone_ids=["zone-a"],
        zone_zu_kanal={"zone-a": 1}, zone_zu_geraet={"zone-a": "dswc-1"},
    )
    # Lauf-Ende 10:10, Toleranz bis 10:25.
    assert _run(job._aktive_bewaesserung_auf_kanal(
        "zone-a", gestartet + timedelta(minutes=20),
    )) is True
    assert _run(job._aktive_bewaesserung_auf_kanal(
        "zone-a", gestartet + timedelta(hours=3),
    )) is False


def test_t0446_soak_pause_blockt_heuristik(speicher):
    """T-0446: waehrend der Soak-Pause ist das Ventil ZU, der Vorgang laeuft
    aber. Ein Feuchte-Sprung in der Pause stammt vom Vorbenetzungs-Puls, fuer
    den bereits ein echtes Event-Paar existiert -- ein Heuristik-Event
    daneben waere die Doppelzaehlung, die dieser Guard verhindern soll.

    Sequenz: Puls 5 min, Haupt-Start bei 30 min, Haupt 60 min -> Ende 90 min.
    Bei 20 min ist Ventil zu (kein live_lauf_state), Sequenz laeuft.
    """
    gestartet = datetime(2026, 4, 19, 10, 0)
    _run(speicher.setze_pre_soak_state(
        zone_id="zone-a", kanal=1, zone_ids_kanal=["zone-a", "zone-b"],
        pre_soak_s=300, pause_s=1800, haupt_s=3600,
        gestartet_am=gestartet, phase="pause",
    ))
    job = SensorBackfillJob(
        speicher, zone_ids=["zone-a", "zone-b"],
        zone_zu_kanal={"zone-a": 1, "zone-b": 1},
        zone_zu_geraet={"zone-a": "dswc-1", "zone-b": "dswc-1"},
    )
    in_pause = gestartet + timedelta(minutes=20)
    assert _run(job._aktive_bewaesserung_auf_kanal("zone-a", in_pause)) is True
    # Die Schwesterzone am selben Kanal ebenso -- Zugehoerigkeit laeuft ueber
    # `zone_ids_kanal`, weil `pre_soak_state` keine `geraet_id` hat.
    assert _run(job._aktive_bewaesserung_auf_kanal("zone-b", in_pause)) is True
    # Nach dem Sequenz-Ende (90 min) plus Toleranz nicht mehr.
    assert _run(job._aktive_bewaesserung_auf_kanal(
        "zone-a", gestartet + timedelta(hours=3),
    )) is False
...
def test_t0175_indoor_zone_skip_regen_check(speicher):
    ...
    _run(speicher.upsert_wetter_archiv(archiv, "beispielstadt"))
...
def test_t0469_cross_spray_wird_als_fremdwasser_geschrieben(speicher):
    """T-0469 (ersetzt T-0312-Unterdrueckung): Hecke-Sprung waehrend
    magerwiese-Lauf OHNE eigenen Heckenlauf -> Event-Paar mit
    ausloser=FREMDWASSER und quell_zone=magerwiese.

    Vorher wurde hier gar nichts geschrieben. Der Sensor hat den Sprung
    real gemessen (29.07.: 44444444 60 -> 70 durch den Rasen-Regner); ein
    leeres `ventil_ereignis` behauptet "ist nie passiert" -- genau die
    Falschaussage, gegen die T-0453 die Klasse FREMDWASSER eingefuehrt hat.
    """
    ...
    assert len(heuristik) == 2, f"OEFFNEN+SCHLIESSEN erwartet: {heuristik}"
    for e in heuristik:
        assert e.ausloser == Ausloser.FREMDWASSER
        assert e.quell_zone == "magerwiese"
    schliessen = [e for e in heuristik if e.aktion == VentilAktion.SCHLIESSEN]
    assert schliessen[0].dauer_sekunden > 0


def test_t0469_fremdwasser_zaehlt_nicht_als_kanal_wasser(speicher):
    """Vertrag der neuen Ereignisse: FREMDWASSER steht in
    KEINE_WASSER_AUSLOESER, damit Bilanz, Tagesbudget, Wirkungs-Fit und ML
    den Cross-Spray weiterhin NICHT als eigene Wassergabe verbuchen. Das
    war die Bedingung dafuer, ihn ueberhaupt schreiben zu duerfen."""
    assert Ausloser.FREMDWASSER in KEINE_WASSER_AUSLOESER


def test_t0469_zweiter_lauf_dupliziert_nicht(speicher):
    """Regression zur Dedup-Klasse: solange unterdrueckt wurde, blieb kein
    Marker liegen -- jeder Backfill-Lauf bewertete denselben Sprung neu.
    Jetzt liegt ein Event da und muss sich beim naechsten Lauf ueber
    `_event_im_fenster` selbst blocken (sonst Duplikat-Flut wie T-0283)."""
    basis = datetime(2026, 6, 20, 10, 0)
    _speichere_messungen(speicher, [
        _m(basis, 45.0, "hecke"),
        _m(basis + timedelta(minutes=30), 60.0, "hecke"),
    ...
    _run(job.aktualisiere_wenn_faellig(jetzt=basis + timedelta(hours=2)))
    job2 = SensorBackfillJob(
        speicher, zone_ids=["hecke"],
        cross_spray_quellen={"hecke": ["magerwiese"]},
    )
    _run(job2.aktualisiere_wenn_faellig(jetzt=basis + timedelta(hours=3)))
    heuristik = [
        e for e in _run(speicher.hole_ventil_ereignisse("hecke"))
        if e.ventil_id == "sensor_heuristik"
    ]
    assert len(heuristik) == 2, f"Genau EIN Paar erwartet: {heuristik}"


def test_t0469_fremdwasser_kaskadiert_nicht_in_die_quell_zone(speicher):
    """Isomorphie-Check zur Klasse "Detektor frisst seine eigene Ausgabe":
    bei WECHSELSEITIG konfigurierten Zonen darf ein FREMDWASSER-Paar der
    einen Zone nicht als "physischer Lauf" der anderen gelten. `_cross_
    spray_quelle` filtert dafuer auf `ventil_id != sensor_heuristik` --
    ohne den Filter wuerde sich Cross-Spray gegenseitig hochschaukeln."""
    basis = datetime(2026, 6, 20, 10, 0)
    _speichere_messungen(speicher, [
        _m(basis, 45.0, "hecke"),
        _m(basis + timedelta(minutes=30), 60.0, "hecke"),
        _m(basis + timedelta(minutes=40), 45.0, "magerwiese"),
        _m(basis + timedelta(minutes=70), 60.0, "magerwiese"),
    ])
    _mw_lauf(speicher, basis + timedelta(minutes=5))
    job = SensorBackfillJob(
        speicher, zone_ids=["hecke", "magerwiese"],
        cross_spray_quellen={
            "hecke": ["magerwiese"], "magerwiese": ["hecke"],
        },
    )
    _run(job.aktualisiere_wenn_faellig(jetzt=basis + timedelta(hours=3)))
    mw = [
        e for e in _run(speicher.hole_ventil_ereignisse("magerwiese"))
        if e.ventil_id == "sensor_heuristik"
    ]
    assert mw, "magerwiese-Sprung muss weiterhin erkannt werden"
    for e in mw:
        assert e.ausloser == Ausloser.UNBEKANNT, (
            "Das Hecke-FREMDWASSER-Paar ist kein physischer Heckenlauf und "
            f"darf magerwiese nicht als Cross-Spray etikettieren: {e}"
        )
        assert e.quell_zone is None
...
def _mw_lauf_mit_ende(speicher, beginn, dauer_min, ausloser=Ausloser.IGNORIERT):
    """Vollstaendiger Magerwiesen-Lauf (OEFFNEN + SCHLIESSEN).

    T-0531 braucht das Laufende: erst damit laesst sich "der Regner lief
    gerade" von "der Regner lief heute frueh mal" unterscheiden.
    """
    _run(speicher.speichere_ventil_ereignis(VentilEreignis(
        zeitstempel=beginn, zone_id="magerwiese", ventil_id="dswc1:1",
        aktion=VentilAktion.OEFFNEN, dauer_sekunden=0, ausloser=ausloser,
    )))
    _run(speicher.speichere_ventil_ereignis(VentilEreignis(
        zeitstempel=beginn + timedelta(minutes=dauer_min), zone_id="magerwiese",
        ventil_id="dswc1:1", aktion=VentilAktion.SCHLIESSEN,
        dauer_sekunden=dauer_min * 60, ausloser=ausloser,
    )))


def _heuristik_events(speicher, zone="hecke"):
    return [
        e for e in _run(speicher.hole_ventil_ereignisse(zone))
        if e.ventil_id == "sensor_heuristik"
    ]


def test_t0531_lauf_lange_vor_dem_sprung_ist_kein_beleg(speicher):
    """Realfall 08.08.2026: Andres Handguss an der Hecke.

    Die Magerwiese lief zuletzt eine knappe Stunde vor dem Feuchtesprung.
    Die Versickerungs-Karenz (3 h) griff trotzdem und stempelte
    `quell_zone=magerwiese` -- eine Ursachenbehauptung, der Andre am 09.08.
    widersprochen hat ("Fremdwasser ist mir null bewusst"). Ein Ops-Eintrag
    mit falscher Ursache ist schlechter als einer, der `unbekannt` sagt.
    """
    basis = datetime(2026, 6, 20, 10, 0)
    _speichere_messungen(speicher, [
        _m(basis, 45.0, "hecke"),
    ...
    # Lauf endet 90 min vor der ersten Messung, also weit vor dem Wasserfenster.
    _mw_lauf_mit_ende(speicher, basis - timedelta(minutes=120), dauer_min=30)
    ...
    events = _heuristik_events(speicher)
    assert events, "Sprung soll weiterhin protokolliert werden"
    for e in events:
        assert e.quell_zone is None, "Vermutung darf nicht als Fakt stehen"
        assert e.ausloser == Ausloser.UNBEKANNT, (
            "ohne belegte Quelle gehoert das Event in die Klassifikation"
        )


def test_t0531_frisch_beendeter_lauf_bleibt_belegt(speicher):
    """Gegenprobe: innerhalb des Nachlaufs bleibt die Zuordnung ein Fakt.

    Sieben der 14 bisherigen Zuordnungen liegen bei 4 bis 16 min nach
    Laufende -- die sollen sich durch T-0531 nicht aendern.
    """
    basis = datetime(2026, 6, 20, 10, 0)
    _speichere_messungen(speicher, [
        _m(basis, 45.0, "hecke"),
    ...
    _mw_lauf_mit_ende(speicher, basis - timedelta(minutes=25), dauer_min=15)
...
def test_t0531_offener_lauf_ohne_close_gilt_als_ursache(speicher):
    """Ein fehlendes SCHLIESSEN heisst "laeuft womoeglich noch", nicht
    "hat nie gewaessert" -- die vorsichtige Richtung. Deckt zugleich den
    Bestand ab, in dem magerwiese-Laeufe nur als OEFFNEN vorliegen."""
    basis = datetime(2026, 6, 20, 10, 0)
    _speichere_messungen(speicher, [
        _m(basis, 45.0, "hecke"),
        _m(basis + timedelta(minutes=30), 60.0, "hecke"),
    ])
    _mw_lauf(speicher, basis + timedelta(minutes=5))  # nur OEFFNEN
    ...
    assert _run(job.aktualisiere_wenn_faellig(jetzt=basis + timedelta(hours=2))) == 1
    for e in _heuristik_events(speicher):
        assert e.quell_zone == "magerwiese"
