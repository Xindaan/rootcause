...
def test_t0557_stunden_grenze_weist_absurde_werte_ab(client_mit_fyta_messung):
    """Plausibilitaetsgrenze auf `stunden` (3 Jahre).

    Reale Aufrufer bleiben unberuehrt -- das groesste Frontend-Fenster ist
    720 h, der Test oben nutzt 10000. Nur die absurden Werte fallen raus.
    """
    assert client_mit_fyta_messung.get(
        "/api/zonen/fyta_topf/messwerte?stunden=1000000",
    ).status_code == 422
    assert client_mit_fyta_messung.get(
        "/api/zonen/fyta_topf/messwerte?stunden=0",
    ).status_code == 422
    # Gegenprobe: der groesste reale Wert muss weiterhin durchgehen.
    assert client_mit_fyta_messung.get(
        "/api/zonen/fyta_topf/messwerte?stunden=720",
    ).status_code == 200
...
def test_t0565_z_zeitstempel_verschiebt_das_fenster_nicht(tmp_path):
    """Das Frontend schickt `Date.toISOString()`, also einen `Z`-String.

    `fromisoformat` akzeptiert den seit Python 3.11 und erzeugt ein AWARE
    datetime; der Speicher bindet es als `'...+00:00'` und vergleicht
    lexikografisch gegen naive Werte. Kein Fehler, nur ein falsches
    Fenster: der "+/- 2 h um das Event"-Chart zeigte in Sommerzeit
    faktisch -4 h bis 0 h, also genau ohne die Sensor-Antwort NACH dem
    Giessen.
    """
    from datetime import timezone

    speicher = Speicher(str(tmp_path / "zeit.db"))
    _run(speicher.verbinden())
    try:
        # Messwerte 08:00 bis 14:00 lokal, alle 30 min.
        basis = datetime(2026, 9, 1, 8, 0)
        for i in range(13):
            _run(speicher.speichere_messung(SensorMessung(
                zeitstempel=basis + timedelta(minutes=30 * i),
                zone_id="fyta_topf", geraet_id="fyta_42",
                boden_feuchte=40.0 + i, boden_temperatur=20.0,
                quelle=DatenQuelle.FYTA,
            )))
        konfiguriere_api(speicher, _konfig(), MagicMock(), MagicMock())
        client = TestClient(app)
        try:
            # Fenster 10:00-14:00 LOKAL, als Z-String ausgedrueckt.
            von_lokal = datetime(2026, 9, 1, 10, 0).astimezone()
            bis_lokal = datetime(2026, 9, 1, 14, 0).astimezone()
            antwort = client.get(
                "/api/zonen/fyta_topf/messwerte",
                params={
                    "von": von_lokal.astimezone(timezone.utc)
                    .isoformat().replace("+00:00", "Z"),
                    "bis": bis_lokal.astimezone(timezone.utc)
                    .isoformat().replace("+00:00", "Z"),
                },
            )
            assert antwort.status_code == 200
            zeiten = [z["zeitstempel"][11:16] for z in antwort.json()]
            assert zeiten[0] == "10:00", f"Fensteranfang falsch: {zeiten[:3]}"
            assert zeiten[-1] == "14:00", f"Fensterende falsch: {zeiten[-3:]}"
            assert len(zeiten) == 9
        finally:
            client.close()
    finally:
