...
async def test_hauptdose_weit_ueberfaellig_wird_als_verfehlt_markiert():
    ...
    # T-0550: Grenze ist jetzt start_s + 3600 s = 30 + 60 = 90 min, unabhaengig
    # von der Dosislaenge (vorher 41 min, weil `haupt_puls_s` mit drinstand).
    # Die Aussage des Tests ist unveraendert: weit ueberfaellig -> kein blindes
    # Feuern, sondern sichtbarer Fehler. Nur "weit" ist jetzt eine andere Zahl.
...
async def test_haupt_fehler_nach_toleranz_setzt_phase_fehler():
    ...
    # T-0550: Fenster-Ende jetzt bei 90 min (start_s 30 + 3600 s), vorher 41.
    await mgr.tick(lauf.gestartet_am + timedelta(minutes=95))
    assert lauf.phase == "fehler"
...
@pytest.mark.asyncio
async def test_status_dict_gestartet_am_traegt_zeitzone():
    """T-0466: `gestartet_am` geht MIT Offset raus.

    Ohne Zonenangabe liest ein Browser ausserhalb der Server-Zeitzone den
    Beispielstadt-naiven Stempel als seine eigene Lokalzeit -- der Pre-Soak-Countdown
    driftet dann um den Zonenversatz. Der Test prueft die Eigenschaft (aware +
    gleicher Zeitpunkt), nicht den konkreten Offset, damit er nicht an der
    Sommerzeit oder der TZ des Testrechners haengt.
    """
    sicherung = VentilSicherungAttrappe()
    mgr = PreSoakManager(sicherung)  # type: ignore[arg-type]
    await mgr.starte(
    ...
    lauf = mgr.laufender_lauf("bambuswald")
    geparst = datetime.fromisoformat(lauf.status_dict()["gestartet_am"])
    assert geparst.tzinfo is not None
    assert geparst.utcoffset() is not None
    # Derselbe Zeitpunkt, nur explizit statt implizit: die naive Server-Zeit
    # zurueckgerechnet muss den Originalstempel treffen.
    assert geparst.astimezone().replace(tzinfo=None) == lauf.gestartet_am
...
async def test_recover_verfehlte_hauptdose_kein_zombie(tmp_path):
    ...
    await sp.verbinden()
    try:
        # T-0550: Nachzug-Grenze ist jetzt pause_s + 3600 s = 90 min und haengt
        # nicht mehr an haupt_s (vorher 41 min). Restart nach 100 min liegt
        # weiterhin klar dahinter -- die Aussage des Tests bleibt.
        gestartet = datetime.now() - timedelta(minutes=100)
        await sp.setze_pre_soak_state(
            zone_id="hecke", kanal=2, zone_ids_kanal=["hecke"],
            pre_soak_s=300, pause_s=1800, haupt_s=60,
            gestartet_am=gestartet, phase="pause",
