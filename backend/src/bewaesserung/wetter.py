...
# T-0563: Sperrzeit nach einem fehlgeschlagenen Abruf (s. `hole_vorhersage`).
FEHLER_CACHE_DAUER = timedelta(minutes=2)
    ...
    async def hole_vorhersage(self) -> WetterVorhersage:
        ...
        async with self._cache_lock:
            ...
            try:
                ...
                # T-0563: kurzes Negativ-Gate. Ohne es blieb
                # `_cache_gueltig_bis` im Fehlerfall ungesetzt, und JEDER
                # Folgeaufruf ging erneut gegen Open-Meteo -- Retry ohne
                # Backoff, waehrend die Entscheidungsschleife pro Zyklus
                # mehrfach fragt. Bewusst viel kuerzer als der Erfolgs-Cache:
                # eine leere Vorhersage soll rasch durch eine echte ersetzt
                # werden, nur eben nicht im Sekundentakt.
                self._cache = WetterVorhersage(
                    abfrage_zeitstempel=jetzt, stunden=[],
                )
                self._cache_gueltig_bis = jetzt + FEHLER_CACHE_DAUER
                return self._cache
