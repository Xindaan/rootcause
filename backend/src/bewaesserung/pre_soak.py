...
# T-0447: Definition liegt in `kanal_zustand`, weil der Kanal-Zustands-Check
# dieselben Endphasen kennen muss. Alias, damit die Aufrufstellen hier
# unveraendert lesbar bleiben.
_END_PHASEN = PRE_SOAK_END_PHASEN

# T-0344: Wie lange nach ihrem Sollzeitpunkt eine noch nicht gelaufene
# Hauptdose vom Tick nachgezogen werden darf. Deckt ab, dass das
# Hauptdose-Fenster kuerzer als der Loop-Tick ist (haupt_s < Tick -> Tick
# springt drueber, fertig schluckt die Dose) oder ein Laptop-Sleep es
# uebersprang. Groesser als ein Tick-Intervall, aber begrenzt: bei groesserer
# Verspaetung ist die Vorbenetzung verpufft -> kein echter Pre-Soak mehr, dann
# als fehler markieren statt eine veraltete Dose zu feuern.
#
# T-0550 (27.08.2026): **flach statt an der Dosislaenge haengend.** Die Formel
# lautete `start_s + haupt_puls_s + 600`. Damit war die erlaubte Verspaetung
# eine Funktion der geplanten Dosis -- fuer bambuswald 3000 s bei Stufe 45 und
# 4800 s bei Stufe 75, also das 1,60-fache. In einem randomisierten Dosis-Test
# haengt so die Ausfallwahrscheinlichkeit an der BEHANDLUNG: kurze Stufen
# brechen strukturell haeufiger ab und fehlen danach in den Daten. Genau das
# ist in T-0535 eingetreten (beide Abbrueche bei 45 und 60, keiner bei 75).
#
# Der Wert 3600 ist an 143 realen Sequenzen seit 01.07. gewaehlt, nicht
# geschaetzt. Gemessene Verspaetung der Hauptdose gegenueber ihrem
# Sollzeitpunkt: 90 % unter 60 s, 95 % unter 300 s, 99 % unter 1800 s,
# Maximum 3076 s. Gegenrechnung, wie viele dieser 143 Laeufe eine Variante
# abgebrochen haette:
#
#     heute (dosisabhaengig, 3000-4800 s)   0
#     flach  600 s                          7
#     flach 1800 s                          1
#     flach 3600 s                          0
#
# 3600 liegt INNERHALB der bisherigen Spanne -- fuer die langen Stufen also
# strenger als vorher, fuer die kurzen grosszuegiger. Die Aenderung glaettet,
# sie verschiebt nicht.
#
# **Ehrlich offen:** der Absatz oben begruendet die Grenze physikalisch mit der
# verpuffenden Vorbenetzung, und eine Stunde passt dazu schlecht. Nur war die
# Zahl auch vorher nicht physikalisch begruendet, sondern ein Nebenprodukt der
# Dosislaenge (Stufe 75 lag bereits bei 4800 s). Wie lange die Vorbenetzung
# wirklich traegt, ist ungemessen; zeigt eine solche Messung, dass sie nach
# ~20 min weg ist, gehoert hier 1800 hin.
#
# T-0553 (29.08.2026, Andres Einwand): **3600 waren zu viel, weil die Groesse
# falsch herum gedacht war.** Gemessen wird die Verspaetung gegenueber dem
# SOLL-START der Hauptdose. Interessant ist aber das ALTER DES PRE-SOAK, denn
# genau dessen Vorbenetzung soll die Hauptdose nutzen. Zwischen Pre-Soak-Ende
# und Soll-Start liegen bei allen Zonen 20 min (`pause_min` 25 minus
# `pre_soak_min` 5); ein Fenster von 3600 erlaubte also einen **80 Minuten
# alten** Pre-Soak. Das ist keine Vorbenetzung mehr.
#
# Der Anlass: wird eine Mikrodrip-Sequenz von einer Zeitfenster-Zone
# verdraengt (T-0552), muss sie hinterher komplett neu ansetzen, Pre-Soak
# eingeschlossen. Das passiert nur, wenn die alte Sequenz vorher SCHEITERT.
# Bei 3600 haing das am Zufall: waldblumenhain belegt den Hahn im Median
# 95 min, aber in 2 von 19 Sequenzen unter 60 min -- dann waere die alte
# Hauptdose mit totem Pre-Soak doch noch gefeuert.
#
# 1200 s ergibt ein maximales Pre-Soak-Alter von 20 + 20 = 40 min, also das
# Doppelte der konstruierten Einsickerzeit. Kosten an denselben 143 Sequenzen
# wie oben: 4 statt 0 Abbrueche (2,8 %). **Diese vier sind kein Verlust,
# sondern der Zweck** -- sie setzen mit frischem Pre-Soak neu an, statt eine
# alte Dose zu feuern.
#
# Die T-0550-Eigenschaft bleibt unberuehrt: der Wert ist weiterhin flach und
# haengt nicht an der Dosislaenge.
_HAUPT_NACHZUG_FENSTER_S = 1200

# Loop-Tick des Entscheidungsloops. Untergrenze fuer das Nachzug-Fenster: waere
# es kleiner, koennte ein Tick ueber das gesamte Fenster springen und `fertig`
# wuerde die Dose schlucken -- der Fall, gegen den T-0344 gebaut wurde. Steht
# hier als Klemme, damit ein spaeter gesenkter Wert das nicht still kaputt
# macht.
_LOOP_TICK_S = 300
...
class PreSoakLauf:
    ...
    def haupt_puls_s(self) -> int:
        ...
        return pre_soak_puls_s(self.haupt_s, self.haupt_pulse)
    ...
    def ende_s(self) -> int:
        ...
        T-0447: Formel in `kanal_zustand`, weil der Kanal-Zustands-Check sie
        auf DB-Zeilen anwenden muss (dort gibt es kein `PreSoakLauf`-Objekt).
        Zwei Kopien der Formel waeren genau die Doppel-Wahrheit, die T-0447
        aufloest.
        """
        return pre_soak_ende_s(
            self.pause_s, self.haupt_s, self.haupt_pulse, self.haupt_pause_s,
        )
    ...
    def status_dict(self) -> dict:
        return {
            ...
            # T-0466: MIT Zonenangabe. `gestartet_am` ist Berlin-naiv (DB-
            # Konvention); ohne Offset interpretiert der Browser den String als
            # seine EIGENE Lokalzeit. In Berlin stimmt das zufaellig, auf einem
            # Handy in einer anderen Zeitzone driftet der Pre-Soak-Countdown --
            # und zwar unauffaellig, weil der Countdown des Live-Laufs daneben
            # korrekt laeuft (`ventil_sicherung.py`, `gestartet`, schon lange
            # mit `.astimezone()`). Klasse: `fehlerpattern_naive_utc_in_db`.
            "gestartet_am": self.gestartet_am.astimezone().isoformat(),
            "fehler": self.fehler,
...
class PreSoakManager:
    ...
    def __init__(
        ...
        haupt_start_callback: Callable[
            ["PreSoakLauf"], Awaitable[None]
        ] | None = None,
    ):
        ...
        # T-0549: wird GENAU EINMAL je Sequenz gerufen, wenn der ERSTE
        # Haupt-Puls tatsaechlich geoeffnet hat. Nicht beim Start der Sequenz:
        # ein Pre-Soak, dem die Hauptdose nie folgt, ist kein gelaufener
        # Vorgang. Realfall T-0535: zwei von 24 Testlaeufen endeten nach dem
        # Pre-Soak (einmal belegte waldblumenhain den Hahn 2,5 h, einmal kam
        # die Hauptdose gar nicht), und beide hatten ihren Zaehler-Slot
        # trotzdem verbraucht -- Blockbalance 7/7/8 statt 8/8/8.
        # Fehler im Callback duerfen die Bewaesserung nicht anhalten.
        self._haupt_start_callback = haupt_start_callback
    ...
    async def _tick_lauf(self, lauf: PreSoakLauf, jetzt: datetime) -> None:
        ...
        if faellig > lauf.haupt_pulse_gestartet:
            ...
            # T-0550: flaches Fenster, bewusst OHNE `haupt_puls_s` -- die
            # erlaubte Verspaetung ist eine Eigenschaft des Bodens und des
            # Loops, keine der geplanten Dosis.
            # T-0553: und knapp genug, dass die Hauptdose keinen toten
            # Pre-Soak mehr nutzt (siehe Konstante).
            grenze = start_s + max(_LOOP_TICK_S, _HAUPT_NACHZUG_FENSTER_S)
            if verstrichen <= grenze:
                ...
                if erfolg:
                    # T-0549: erster Haupt-Puls dieser Sequenz? Dann ist ab
                    # hier die Hauptdose tatsaechlich gelaufen. `index >= 2`
                    # meldet nicht erneut, und ein aus der Persistenz
                    # wiederhergestellter Lauf traegt oben schon
                    # `haupt_pulse_gestartet = 1` -- er meldet also auch nach
                    # einem Neustart nicht doppelt.
                    erster_haupt_puls = lauf.haupt_pulse_gestartet == 0
                    lauf.phase = "haupt"
                    ...
                    if erster_haupt_puls and self._haupt_start_callback:
                        try:
                            await self._haupt_start_callback(lauf)
                        except Exception:
                            logger.exception(
                                "pre_soak.haupt_start_callback_fehler",
                                zone_id=lauf.zone_id,
                                lauf_gruppe=lauf.lauf_gruppe or None,
                            )
                else:
