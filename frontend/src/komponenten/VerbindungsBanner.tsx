...
export function VerbindungsBanner({ fehler, letzterErfolg }: Props) {
  ...
  useEffect(() => {
    // T-0460: starteSichtbarkeitsIntervall aktualisiert `jetzt` beim
    // Zurueckkehren aus dem Hintergrund sofort -- deckt "beim Aufwachen
    // sofort sichtbar" (s. Kopfkommentar) mindestens so gut ab wie vorher.
    const stoppeIntervall = starteSichtbarkeitsIntervall(() => setJetzt(Date.now()), 30_000)
