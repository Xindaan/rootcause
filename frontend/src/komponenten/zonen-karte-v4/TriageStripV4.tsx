...
function bucketVon(zone: Zone, giesst: boolean, hatEmpfehlung: boolean): Bucket {
  ...
  // T-0532: Lead-Ausfall gehoert in dieselbe Klasse -- die Zahl ist da, sie
  // sagt nur nichts ueber die Zone. Ohne diese Zeile buckte der Strip
  // "1 kritisch", waehrend das Band daneben "Lead ausgefallen" meldet.
  if (ist == null || istWahrscheinlichSensorDefekt(zone) || leadAusgefallen(zone)) {
    return 'unbekannt'
  }
  ...
  // T-0491: null = keine Obergrenze, also kein nass-Bucket.
  const maxSchwelle = zone.feuchte_schwelle_max
  if (maxSchwelle != null && ist > maxSchwelle) return 'nass'
