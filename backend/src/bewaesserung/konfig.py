...
def _parse_zonen(roh_zonen: list[dict]) -> list[ZonenKonfig]:
    ...
    for roh in roh_zonen:
        ...
        # T-0576 E: datengetriebenes Giessfenster. Diese Zeile ist die
        # Whitelist -- ohne sie kommt der YAML-Block still als None an
        # (fehlerpattern_config_whitelist). `klammer` im selben Kurzformat
        # wie `bevorzugte_zeiten` ("HH:MM-HH:MM").
        giessfenster_et0 = None
        if roh.get("giessfenster_et0"):
            gf = dict(roh["giessfenster_et0"])
            gf["klammer"] = [_parse_zeitfenster(z) for z in gf.get("klammer", [])]
            giessfenster_et0 = GiessfensterEt0Konfig(**gf)
        ...
        zonen.append(ZonenKonfig(
            ...
            # T-0512: ohne diese Zeile bliebe das Feld stumm auf dem
            # Pydantic-Default (fehlerpattern_config_whitelist).
            ml_dosis_opt_in=roh.get("ml_dosis_opt_in", False),
            # T-0492: dasselbe Whitelist-Pattern -- ohne diese Zeile bleibt
            # das Feld trotz YAML-Eintrag None, und alle historischen
            # `automatik`-Events fielen weiter aus dem Training.
            auto_loop_scharf_seit=roh.get("auto_loop_scharf_seit"),
            ...
            # T-0540: Opt-out fuer die "Bewaesserung ohne Wirkung"-Pruefung
            # dieser Zone (Default true = pruefen). Nur diese eine Pruefung,
            # die uebrigen Leck-Detektor-Checks bleiben aktiv.
            wirkungs_alarm_aktiv=bool(roh.get("wirkungs_alarm_aktiv", True)),
            # T-0574: Opt-out fuer die "Sensor eingefroren"-Pruefung dieser
            # Zone (Default true = pruefen). Nur diese eine Pruefung.
            eingefroren_alarm_aktiv=bool(
                roh.get("eingefroren_alarm_aktiv", True)
            ),
            ...
            # T-0439: None -> globale Engine-Konstante REGEN_FEUCHTE_FAKTOR.
            regen_faktor_pp_pro_mm=roh.get("regen_faktor_pp_pro_mm"),
            # T-0440: Zwiebelsensor darf den Kanal-Lauf nicht beenden.
            kanal_max_stop_ausschluss=roh.get("kanal_max_stop_ausschluss", False),
            # T-0444: Notbremse fuer die ausgeschlossene Zone. Whitelist-
            # Pattern (Memory fehlerpattern_config_whitelist) -- ohne diese
            # Zeile bliebe der YAML-Wert stumm liegen.
            kanal_max_stop_notbremse_pp=float(
                roh.get("kanal_max_stop_notbremse_pp", 95.0),
            ),
            # T-0434: 0.0-Kontaktverlust-Guard, Default an. Gleiches
            # Whitelist-Pattern (Memory fehlerpattern_config_whitelist) --
            # ohne diese Zeile liesse sich der Guard per YAML nicht
            # abschalten, das Feld waere stumm.
            feuchte_null_ist_defekt=bool(
                roh.get("feuchte_null_ist_defekt", True),
            ),
            ...
            regen_erreicht_ballen=roh.get("regen_erreicht_ballen", True),
...
def _parse_datum(roh_wert) -> "date | None":
    """T-0535: 'YYYY-MM-DD' (oder bereits geparstes YAML-Datum) -> date."""
    if roh_wert is None:
        return None
    if isinstance(roh_wert, datetime):
        return roh_wert.date()
    if isinstance(roh_wert, date):
        return roh_wert
    return date.fromisoformat(str(roh_wert))


def _parse_dosis_test(roh: dict) -> DosisTestKonfig:
    """T-0535: Top-Level-Block `dosis_test:` -> DosisTestKonfig.

    Whitelist-Pattern (Memory: fehlerpattern_config_whitelist.md) -- ohne
    diesen Parser bliebe der YAML-Block stumm liegen und der Test waere
    per Config nicht einschaltbar. Fehlt der Block ganz, entsteht der
    Default `aktiv=False` (exakter No-op).
    """
    block = roh.get("dosis_test") or {}
    kanal_roh = block.get("ventil_kanal")
    return DosisTestKonfig(
        aktiv=bool(block.get("aktiv", False)),
        ventil_geraet_id=str(block.get("ventil_geraet_id") or ""),
        ventil_kanal=int(kanal_roh) if kanal_roh is not None else None,
        stufen_gesamt_min=tuple(
            int(s) for s in (block.get("stufen_gesamt_min") or [])
        ),
        wiederholungen=int(block.get("wiederholungen", 0)),
        seed=int(block.get("seed", 0)),
        bis=_parse_datum(block.get("bis")),
    )
...
def lade_konfig(pfad: Path | None = None) -> GesamtKonfig:
    ...
    ml_retrain_konfig = MlRetrainKonfig(
        ...
        # T-0403: ohne diese Zeile blieb `feature_bau_subprozess: true` aus der
        # YAML wirkungslos -- der Modell-Default ist bewusst False (sonst
        # laesen Tests die Produktiv-DB), also gewinnt er, solange ihn niemand
        # ueberschreibt. Belegt am Retrain 13.08. 11:09-11:23: kein
        # `ml.retrain.features_aus_subprozess` im Log, Entscheidungsloop
        # stockte 240 s. Der Fix war deployed und tat nichts.
        # Memory: fehlerpattern_config_whitelist.
        feature_bau_subprozess=retrain_roh.get(
            "feature_bau_subprozess",
            retrain_defaults.feature_bau_subprozess,
        ),
        # Isomorphie-Fund derselben Durchsicht: auch dieses Feld fehlte. YAML
        # sagt 5 Minuten, gewirkt hat der Default 30 -- deshalb kam der Retrain
        # nach einem Neustart erst nach ~45 min statt nach ~20.
        start_verzoegerung_minuten=retrain_roh.get(
            "start_verzoegerung_minuten",
            retrain_defaults.start_verzoegerung_minuten,
        ),
    )
    ...
    ml_bewaesserungs_response_konfig = MlBewaesserungsResponseKonfig(
        ...
        min_events_pump_zone=response_roh.get(
            "min_events_pump_zone", response_defaults.min_events_pump_zone,
        ),
        ...
        # T-0539: derselbe Schalter wie im Feuchte-Pfad, und dieselbe Falle --
        # der Modell-Default ist bewusst False, also bleibt die YAML ohne diese
        # Zeile wirkungslos. In T-0403 hat genau das einen halben Tag gekostet:
        # der Fix war deployed, gemessen hat sich nichts geaendert.
        feature_bau_subprozess=response_roh.get(
            "feature_bau_subprozess", response_defaults.feature_bau_subprozess,
        ),
    )
    ...
    return GesamtKonfig(
        ...
        wetter=WetterKonfig(
            ...
            # T-0439: wirkungsbasiertes Regen-Gate.
            regen_gate_modus=wetter_roh.get("regen_gate_modus", "wirkung"),
            regen_gate_immer_ab_mm=wetter_roh.get("regen_gate_immer_ab_mm", 10.0),
            # T-0509: Archiv-Produkt explizit. Ohne diesen Schluessel liefert
            # die Archive-API `best_match` (= hier ecmwf_ifs).
            archiv_modell=wetter_roh.get("archiv_modell", "era5"),
        ),
        ...
        # Stand 18.09.2026: fehlte hier, der YAML-Wert wurde still ignoriert
        # und der Modell-Default (24 h) gewann immer. Default bewusst aus dem
        # Modell gelesen statt als zweite Literal-Kopie.
        plant_optimum_intervall_stunden=roh.get(
            "plant_optimum_intervall_stunden",
            GesamtKonfig.model_fields["plant_optimum_intervall_stunden"].default,
        ),
        ...
        # T-0535: randomisierter Dosis-Test (Default inaktiv).
        dosis_test=_parse_dosis_test(roh),
