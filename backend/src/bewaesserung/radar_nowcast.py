...
class Nowcast:
    punkt_mm: float
    # T-0568: der Name sagt "max", der Wert ist die SUMME der Schritt-Maxima
    # ueber das Vorhersagefenster (s. `werte_aus_gitter`). Der Docstring dort
    # sagt es richtig, der Feldname nicht -- und `veto(schwelle_mm=1.0)` liest
    # sich dadurch wie "irgendwo mehr als 1 mm", waehrend es "in Summe ueber
    # zwei Stunden mehr als 1 mm" bedeutet.
    #
    # BEWUSST NICHT UMBENANNT: dieses Modul hat repo-weit keinen
    # Produktiv-Konsumenten (nur `test_wetter_ensemble.py` importiert es).
    # Ein Rename waere Churn an totem Code; die Entscheidung "verdrahten oder
    # entfernen" steht als eigener Punkt in TASK.md.
    umfeld_max_mm: float
    schritte: int
