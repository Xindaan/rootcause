...
class MLResponseService:
    ...
    def inverse_dauer(
        ...
        *,
        ...
        # T-0564: unbekannte Feature-Spalten sind ein Schema-Bruch, kein
        # fehlender Messwert. Sie mit 0.0 zu imputieren erzeugt eine Dauer,
        # die wie ein Ergebnis aussieht -- reproduziert: zwei von drei
        # Spalten unbekannt und das Modell lieferte 928 Sekunden. Der
        # Aufrufer (`entscheidung._dauer_mit_ml_weiche`) clippt nur auf
        # `[MIN_DAUER, max_dauer_sekunden]` und wuerde das bei
        # `ml_dosis_wirksam` als echtes Wasser fahren. Der Docstring nennt
        # diesen Fall seit jeher als `None`-Fall; der Code tat es nicht.
        unbekannt = [c for c in feature_cols if c not in feature_werte]
        if unbekannt:
            # ACHTUNG stdlib-logger in diesem Modul: KEINE structlog-kwargs
            # ([[fehlerpattern_stdlib_structlog_mix]]).
            logger.warning(
                "ml.response.feature_schema_bruch zone=%s fehlend=%s "
                "anzahl=%d -- kein Dauer-Vorschlag, Heuristik uebernimmt",
                zone_id, unbekannt[:5], len(unbekannt),
            )
            return None
