...
# T-0453: FREMDWASSER gehoert hierher -- der Feuchte-Sprung ist echt, aber
# das Wasser kam aus dem Kanal einer Nachbar-Zone. Genau der Fall, den der
# Modul-Docstring oben als "Cross-Spray/Phantom" beschreibt; bis T-0453 gab
# es dafuer keinen Ausloeser-Wert, sondern nur den `ignoriert`-Umweg.
# ZEITPLAN (T-0455) steht bewusst NICHT hier: der Cloud-Zeitplan ist ein
# realer Lauf dieses Kanals und muss in der Historie sichtbar mitzaehlen.
_NICHT_ZAEHLEND = {
...
def _manueller_log_puls(o: VentilEreignis) -> dict:
    """T-0566: Ein nachtraeglich geloggter Handguss als fertiger Puls.

    Traegt seine Dauer selbst (Single-Event-Modus). `ende` wird aus
    `start + dauer` gebildet -- das ist der protokollierte Zeitraum, nicht
    ein gemessener Close.
    """
    ...
    return {
        "start": o.zeitstempel,
        "ende": (
            o.zeitstempel + timedelta(seconds=o.dauer_sekunden)
            if o.dauer_sekunden > 0 else o.zeitstempel
        ),
        "dauer_s": o.dauer_sekunden,
...
def gruppiere_giess_laeufe(ereignisse: list[VentilEreignis]) -> list[dict]:
    ...
    for e in evs:
        if e.aktion == VentilAktion.OEFFNEN:
            # T-0566: Das Einzel-OEFFNEN aus `POST /api/giessen` ist ein
            # VOLLSTAENDIGER Eintrag, kein offener Lauf. Der Nutzer loggt
            # eine Giesskannen-/Schlauch-Aktion nachtraeglich; ein
            # SCHLIESSEN folgt bewusst nie ([[ventil_event_vertrag]],
            # Ergaenzung 17.04.2026). Ohne diesen Zweig landete es in
            # `_offener_puls_dict`, das `dauer_s: None` setzt -- die vom
            # Nutzer eingetragene Dauer verschwand aus der Historie und
            # `laeuft_noch` blieb dauerhaft True. Aufraeumen konnte das
            # niemand: `hole_orphan_oeffnen` und
            # `hole_offene_ventil_kanaele` filtern auf
            # `ventil_id GLOB '*:[0-9]*'`, und `manuell` matcht da nicht.
            if e.dauer_sekunden > 0 or e.liter is not None:
                pulse.append(_manueller_log_puls(e))
                continue
