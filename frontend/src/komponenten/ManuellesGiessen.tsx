...
function verbleibendSec(lauf: PreSoakLauf): number {
  ...
  if (lauf.phase === 'haupt') {
    const hauptStart = lauf.pause_s
    return Math.max(0, lauf.haupt_s - (verstrichen - hauptStart))
  }
  return 0
}
...
export function ManuellesGiessen({
  ...
  const [ventilLauf, setVentilLauf] = useState<
  ...
  useEffect(() => {
      ...
      try {
        ...
        for (const eintrag of Object.values(r.aktiv ?? {})) {
          if (eintrag.zone_ids?.includes(zoneId)) {
            setVentilLauf({ kind: 'backend', eintrag })
        ...
        for (const eintrag of Object.values(r.extern ?? {})) {
          if (eintrag.zone_ids?.includes(zoneId)) {
            setVentilLauf({ kind: 'extern', eintrag })
            setVentilStatusFehler(null)
            return
          }
        }
    ...
    void fetchStatus()
    const cadence = ventilLauf != null ? 5000 : 30000
    const id = setInterval(fetchStatus, cadence)
    return () => {
      aktiv = false
      ctrl.abort()
      clearInterval(id)
    }
  }, [zoneId, hatVentilKanal, ventilLauf])
  ...
  useEffect(() => {
    ...
    void fetchStatus()
    const cadence = psLauf != null ? 5000 : 30000
    const id = setInterval(fetchStatus, cadence)
    return () => {
      aktiv = false
      ctrl.abort()
  ...
  if (ventilLauf && ventilLauf.kind === 'backend' && !psLauf) {
    void tick
    const verbleibend = ventilLauf.eintrag.verbleibend_s ?? 0
  ...
  if (psLauf) {
        ...
        <>
          ...
          <div className="giessen-container giessen-auswahl-live mg-lauf mg-lauf--fremd">
            <span className="giessen-label">
              ...
              {phaseLabel(psLauf.phase)} ({countdownText})
            </span>
          </div>
        </>
      )
    ...
    return (
      <>
        ...
        <div className="giessen-container giessen-auswahl-live mg-lauf">
          <span className="giessen-label">
            {istAutomatik ? 'Automatik' : 'Pre-Soak'}: {phaseLabel(psLauf.phase)} ({countdownText})
            {committetMin != null && (
