...
class FeatureExtraktor:
    ...
    def _baue_dataframe_sync(
        ...
        """
        messungen_pro_zone = self._gruppiere_messungen(messungen)
        ventile_pro_zone = self._gruppiere_ventile(ventil_ereignisse)

        zeilen = []
        for zone_id, zone_messungen in messungen_pro_zone.items():
    ...
    def _gruppiere_messungen(
        ...
        """Gruppiert Messungen nach Zone, chronologisch sortiert."""
        gruppen: dict[str, list[SensorMessung]] = defaultdict(list)
        for m in messungen:
            gruppen[m.zone_id].append(m)
        # Chronologisch sortieren (aelteste zuerst)
        for zone_id in gruppen:
            gruppen[zone_id].sort(key=lambda m: m.zeitstempel)
        return gruppen
    ...
    def _baue_zone_features(
        ...
        for i, messung in enumerate(messungen):
            ...
            for horizont in self.HORIZONTE:
                ...
                # Wind-Match als separater Baustein
                wind_match = _wind_match_ordinal(zone, standort_id, self._konfig)
                zeile[f"wind_match"] = wind_match
